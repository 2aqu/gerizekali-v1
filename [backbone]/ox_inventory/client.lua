...
local function canOpenInventory()
	...
	and not exports["efearob-base"]:CuffCheck()
	and invOpen ~= nil
...
function client.openInventory(inv, data)
	if exports["progressbar"]:isAction() == false then
		if inv == "player" then
		...
		if durability and durability <= 100 then
			if durability <= 0 then
				if string.match(item.name, "WEAPON_") then
					...
					TriggerServerEvent('arob:removeitem', item.name, 1, item.slot)
				end
							...
							if data.name == currentWeapon.ammo then
								if bekleamk == false then
									...
									if newAmmo < maxAmmo then
										...
										TriggerServerEvent('arob:removeitem', data.name, 1)
										bekleamk = true
										Wait(1200)
										bekleamk = false
										if cache.vehicle then
