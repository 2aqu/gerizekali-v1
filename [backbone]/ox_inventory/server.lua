...
lib.callback.register('ox_inventory:useItem', function(source, itemName, slot, metadata)
	...
	if inventory.player then
		...
		if item and data and data.count > 0 and data.name == item.name then
			...
			local Player = QBCore.Functions.GetPlayerByCitizenId(inventory.owner)
...
lib.addCommand({'additem', 'giveitem'}, {
	...
	if item then
		...
		-- if server.loglevel > 0 then
			...
			TriggerEvent('qb-log:server:CreateLog', "giveitemadmin", "GiveItem", "default", "giveitem " .. source.label .. " " .. item.name .. " " .. count)
		-- end
	end
end)
...
RegisterServerEvent('envanterbussy-sv')
AddEventHandler('envanterbussy-sv', function(idamk, acik)
	TriggerClientEvent('envanterbussy', idamk, acik)
end)