...
local function nearbyShop(point)
	...
	if point.isClosest and point.currentDistance < 1.2 then
		DrawText3Ds(point.coords.x, point.coords.y, point.coords.z + 0.3, "[E]")
	end
			...
			elseif shop.targets then
				for i = 1, #shop.targets do
					...
					elseif target.loc then
						...
						if target.pedaq then
							-- exports["arob-main"]:pedcreate(shopid, `ig_tomepsilon`, target.loc.x, target.loc.y, target.loc.z - 1, target.heading or 0.0)
						end
...
RegisterCommand('testenv', function()
	refreshShops()
end)
