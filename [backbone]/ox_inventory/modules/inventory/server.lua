...
local function randomLoot(loot)
	...
	if math.random(1,100) <= 40 then
		local randomaq = math.random(1, #randomitems)
		items[#items+1] = {randomitems[randomaq], 1}
	end
...
function Inventory.AddItem(inv, item, count, metadata, slot, cb)
	...
	--print("debug", item)
	if type(item) ~= 'table' then item = Items(item) end
	local itemagirlik = item.weight or 3500
	local itemagirlik = item.weight * count

	if not item then return false, 'invalid_item' end

	inv = Inventory(inv) --[[@as OxInventory]]
	
	if inv.weight + itemagirlik < 100000 then
		...
		return success, responsee
	else
		TriggerClientEvent('QBCore:Notify', src, "Envanterin dolu!", "error")
		return false, false
	end
end
...
function Inventory.RemoveItem(inv, item, count, metadata, slot, ignoreTotal)
	...
	if count > 0 then
		...
		if removed > 0 then
			...
			-- if invokingResource then
				-- lib.logger(inv.owner, 'removeItem', ('"%s" kullandı %sx %s from "%s"'):format(item.name, removed, item.name, inv.label))
			-- end
...
local function dropItem(source, data)
	...
	-- print(1)
		local Player = QBCore.Functions.GetPlayerByCitizenId(playerInventory.owner)
		local isim = Player.PlayerData.charinfo["firstname"]
		local soyisim = Player.PlayerData.charinfo["lastname"]
		local citizenid = Player.PlayerData.citizenid
		local steam = "Kullanılmıyor"
		local discord = "Bilinmiyor"
		for k, v in ipairs(GetPlayerIdentifiers(Player.PlayerData.source)) do
		...
		embed = {{
			...
			["fields"] = {
				...
				[2] = {
					["name"] = "Steam İsmi",
					["value"] = ""..GetPlayerName(Player.PlayerData.source),
					["inline"] = true
				},
				[3] = {
					["name"] = "Steam Hex",
		...
		PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
	-- end
			...
			elseif data.count <= fromData.count then
				...
				if fromInventory.type == 'container' or sameInventory or (toInventory.weight + toData.weight <= toInventory.maxWeight) then
					...
					if not sameInventory then
						...
						-- if server.loglevel > 0 then
						if string.match(fromInventory.id, "drop-") then
							local Player = QBCore.Functions.GetPlayerByCitizenId(playerInventory.owner)
							local isim = Player.PlayerData.charinfo["firstname"]
							local soyisim = Player.PlayerData.charinfo["lastname"]
							local citizenid = Player.PlayerData.citizenid
							...
							PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
						elseif toInventory.owner and fromInventory.owner then
							local Player = QBCore.Functions.GetPlayerByCitizenId(toInventory.owner)
							if Player then
								local isim = Player.PlayerData.charinfo["firstname"]
								local soyisim = Player.PlayerData.charinfo["lastname"]
								local citizenid = Player.PlayerData.citizenid
								...
								local Player2 = QBCore.Functions.GetPlayerByCitizenId(fromInventory.owner)
								if Player2 then
									local isim2 = Player2.PlayerData.charinfo["firstname"]
									local soyisim2 = Player2.PlayerData.charinfo["lastname"]
									local citizenid2 = Player2.PlayerData.citizenid
									local steam2 = "Kullanılmıyor"
									local discord2 = "Bilinmiyor"
									for k, v in ipairs(GetPlayerIdentifiers(Player2.PlayerData.source)) do
										if string.match(v, "discord:") then
											discord2 = v
											break
										end
									...
									embed = {{
										...
										["fields"] = {
											[1] = {
												["name"] = "Üstten Eşya Al - Bırak",
												["value"] = "",
												["inline"] = false
											},
											[2] = {
												["name"] = "Eşyayı Alanın Steam İsmi",
												["value"] = ""..GetPlayerName(Player.PlayerData.source),
												["inline"] = true
											},
											[3] = {
												["name"] = "Eşyayı Alanın Steam Hex",
												["value"] = ""..steam,
												["inline"] = true
											},
											[4] = {
												["name"] = "Eşyayı Alanın Discord",
												["value"] = "<@"..discord:gsub('discord:', '') .. ">",
												["inline"] = true
											},
											[5] = {
												["name"] = "Eşyayı Alanın Citizen ID",
												["value"] = ""..citizenid,
												["inline"] = true
											},
											[6] = {
												["name"] = "Eşyayı Alanın Oyun İçi ID",
												["value"] = ""..Player.PlayerData.source,
												["inline"] = true
											},
											[7] = {
												["name"] = "Eşyayı Alanın Ad Soyad",
												["value"] = ""..isim .. " " .. soyisim,
												["inline"] = true
											},
											[8] = {
												["name"] = "Eşya Alınan Kişinin Steam İsmi",
												["value"] = ""..GetPlayerName(Player2.PlayerData.source),
												["inline"] = true
											},
											[9] = {
												["name"] = "Eşya Alınan Kişinin Steam Hex",
												["value"] = ""..steam2,
												["inline"] = true
											},
											[10] = {
												["name"] = "Eşya Alınan Kişinin Discord",
												["value"] = "<@"..discord2:gsub('discord:', '') .. ">",
												["inline"] = true
											},
											[11] = {
												["name"] = "Eşya Alınan Kişinin Citizen ID",
												["value"] = ""..citizenid2,
												["inline"] = true
											},
											[12] = {
												["name"] = "Eşya Alınan Kişinin Oyun İçi ID",
												["value"] = ""..Player2.PlayerData.source,
												["inline"] = true
											},
											[13] = {
												["name"] = "Eşya Alınan Kişinin Ad Soyad",
												["value"] = ""..isim2 .. " " .. soyisim2,
												["inline"] = true
											},
											[14] = {
												["name"] = "Alınan Eşya",
									...
									local Player = QBCore.Functions.GetPlayerByCitizenId(toInventory.owner)
									if fromInventory.id == Player.PlayerData.source then
										local isim = Player.PlayerData.charinfo["firstname"]
										local soyisim = Player.PlayerData.charinfo["lastname"]
										local citizenid = Player.PlayerData.citizenid
										local steam = "Kullanılmıyor"
										...
										if toInventory.type == "glovebox" then
											PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
										...
										if fromInventory.type == "glovebox" then
											PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
										elseif fromInventory.type == "trunk" then
											PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
										else
											PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
										end
								...
								local Player = QBCore.Functions.GetPlayerByCitizenId(fromInventory.owner)
								if fromInventory.id == Player.PlayerData.source then
									local isim = Player.PlayerData.charinfo["firstname"]
									local soyisim = Player.PlayerData.charinfo["lastname"]
									local citizenid = Player.PlayerData.citizenid
		...
		if TriggerEventHooks('swapItems', {
			...
			if Inventory.AddItem(toInventory, item, count, data.metadata, toSlot) then
				if Inventory.RemoveItem(fromInventory, item, count, data.metadata, slot) then
					-- if server.loglevel > 0 then
					-- print(5)
						local Player = QBCore.Functions.GetPlayer(fromInventory.id)
						local isim = Player.PlayerData.charinfo["firstname"]
						local soyisim = Player.PlayerData.charinfo["lastname"]
						local citizenid = Player.PlayerData.citizenid
						local steam = "Kullanılmıyor"
						...
						local discord = "Bilinmiyor"
						for k, v in ipairs(GetPlayerIdentifiers(fromInventory.id)) do
							if string.match(v, "discord:") then
								discord = v
								break
							end
						end
						local Player2 = QBCore.Functions.GetPlayer(toInventory.id)
						local isim2 = Player2.PlayerData.charinfo["firstname"]
						local soyisim2 = Player2.PlayerData.charinfo["lastname"]
						local citizenid2 = Player2.PlayerData.citizenid
						local steam2 = "Kullanılmıyor"
						local discord2 = "Bilinmiyor"
						for k, v in ipairs(GetPlayerIdentifiers(toInventory.id)) do
							if string.match(v, "discord:") then
								discord2 = v
								break
							end
						...
						embed = {{
							...
							["fields"] = {
								...
								[2] = {
									["name"] = "Veren Kişinin Steam İsmi",
									["value"] = ""..GetPlayerName(fromInventory.id),
									["inline"] = true
								},
								[3] = {
									["name"] = "Veren Kişinin  Steam Hex",
									["value"] = ""..steam,
									["inline"] = true
								},
								[4] = {
									["name"] = "Veren Kişinin  Discord",
									["value"] = "<@"..discord:gsub('discord:', '') .. ">",
									["inline"] = true
								},
								[5] = {
									["name"] = "Veren Kişinin  Citizen ID",
									["value"] = ""..citizenid,
									["inline"] = true
								},
								[6] = {
									["name"] = "Veren Kişinin  Oyun İçi ID",
									["value"] = ""..fromInventory.id,
									["inline"] = true
								},
								[7] = {
									["name"] = "Veren Kişinin  Ad Soyad",
									["value"] = ""..isim .. " " .. soyisim,
									["inline"] = true
								},
								[8] = {
									["name"] = "Alan Kişinin Steam İsmi",
									["value"] = ""..GetPlayerName(toInventory.id),
									["inline"] = true
								},
								[9] = {
									["name"] = "Alan Kişinin  Steam Hex",
									["value"] = ""..steam2,
									["inline"] = true
								},
								[10] = {
									["name"] = "Alan Kişinin  Discord",
									["value"] = "<@"..discord2:gsub('discord:', '') .. ">",
									["inline"] = true
								},
								[11] = {
									["name"] = "Alan Kişinin  Citizen ID",
									["value"] = ""..citizenid2,
									["inline"] = true
								},
								[12] = {
									["name"] = "Alan Kişinin  Oyun İçi ID",
									["value"] = ""..toInventory.id,
									["inline"] = true
								},
								[13] = {
									["name"] = "Alan Kişinin  Ad Soyad",
									["value"] = ""..isim2 .. " " .. soyisim2,
									["inline"] = true
								},
								[14] = {
						...
						PerformHttpRequest("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", function() end, 'POST', json.encode({ username = 'Inventory Log', embeds = embed}), { ['Content-Type'] = 'application/json' })
						TriggerClientEvent('arob:me-do', src, count .. " adet " .. data.label .. " uzatır.")
						TriggerClientEvent('arob:me-do', trgt, "uzatılan " .. count .. " adet " .. data.label .. "'i alır.")
					-- end
