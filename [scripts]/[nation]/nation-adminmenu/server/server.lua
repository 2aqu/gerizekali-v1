...
QBCore.Functions.CreateCallback('qb-admin:server:getrank', function(source, cb)
    if QBCore.Functions.HasPermission(source, 'mod') or IsPlayerAceAllowed(source, 'command') then
        cb(true)
    else
        cb(false)
    end
...
QBCore.Functions.CreateCallback('qb-admin:server:getrankAd', function(source, cb)
    if QBCore.Functions.HasPermission(source, 'support') or IsPlayerAceAllowed(source, 'command') then
        cb(true)
    else
        cb(false)
    end
...
-- Events
RegisterNetEvent('qb-admin:server:SendLog', function(type, color, text, player)
    if player then
        src = player
    else
        src = source
    ...
    TriggerEvent('qb-log:server:CreateLog', type, 'Admin Log', color, '**'..GetPlayerName(src)..'** (CitizenID: '..Player.PlayerData.citizenid..' | Isim: '.. Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname ..' | ID: '..src..') \n** '.. text ..' **')
end)

RegisterNetEvent('qb-admin:server:SendServerLog', function(src, other, type, color, text)
    local Player = QBCore.Functions.GetPlayer(src)
    local OtherPlayer = QBCore.Functions.GetPlayer(other)
    if Player and OtherPlayer then
        TriggerEvent('qb-log:server:CreateLog', type, 'Admin Log', color, '**'..GetPlayerName(src)..'** (CitizenID: '..Player.PlayerData.citizenid..' | Isim: '.. Player.PlayerData.charinfo.firstname .. ' ' .. Player.PlayerData.charinfo.lastname ..' | ID: '..src..') \n (CitizenID: '..OtherPlayer.PlayerData.citizenid..' | Isim: '.. OtherPlayer.PlayerData.charinfo.firstname .. ' ' .. OtherPlayer.PlayerData.charinfo.lastname ..' | ID: '..other..') '.. text)
    end
end)
...
RegisterNetEvent('qb-admin:server:kill', function(player)
    ...
    TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'kill', 'red', 'isimli oyuncuyu öldürdü.')
end)

RegisterNetEvent('qb-admin:server:revive', function(player)
    TriggerClientEvent('hospital:client:Revive', player.id)
    TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'revive', 'green', 'isimli oyuncuya revive attı.')
end)
...
RegisterNetEvent('qb-admin:server:kick', function(player, reason)
    ...
    if QBCore.Functions.HasPermission(src, permissions['kick']) or IsPlayerAceAllowed(src, 'command')  then
        TriggerEvent('qb-log:server:CreateLog', 'bans', 'Oyuncu Sunucudan Atıldı', 'yellow', string.format('%s isimli oyuncu %s tarafından %s sebebiyle sunucudan atıldı.', GetPlayerName(player.id), GetPlayerName(src), reason))
...
RegisterNetEvent('qb-admin:server:ban', function(player, time, reason)
    ...
    if QBCore.Functions.HasPermission(src, permissions['ban']) or IsPlayerAceAllowed(src, 'command') then
        ...
        TriggerEvent('qb-log:server:CreateLog', 'bans', 'Oyuncu Sunucudan Uzaklaştırıldı', 'red', string.format('%s isimli oyuncu %s tarafından %s sebebiyle sunucudan uzaklaştırıldı.', GetPlayerName(player.id), GetPlayerName(src), reason))
        if banTime >= 2147483647 then
...
RegisterNetEvent('qb-admin:server:spectate', function(player)
    ...
    TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'spectate', 'green', 'isimli oyuncuyu izlemeye başladı.')
end)
...
RegisterNetEvent('qb-admin:server:freeze', function(player)
    ...
    if not frozen then
        ...
        TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'freeze', 'red', 'isimli oyuncuyu dondurdu.')
    else
        frozen = false
        FreezeEntityPosition(target, false)
        TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'freeze', 'green', 'isimli oyuncunun dondurulmasını kaldırdı.')
    end
end)
...
RegisterNetEvent('qb-admin:server:goto', function(player)
    ...
    TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'goto', 'green', 'isimli oyuncunun yanına ışınlandı.')
end)
...
RegisterNetEvent('qb-admin:server:bring', function(player)
    ...
    TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'bring', 'green', 'isimli oyuncuyu yanına çekti.')
end)

RegisterNetEvent('qb-admin:server:inventory', function(player)
    local src = source
    TriggerClientEvent('qb-admin:client:inventory', src, player.id)
    TriggerEvent('qb-admin:server:SendServerLog', source, player.id, 'invopen', 'green', 'isimli oyuncunun envanterini açtı.')
end)
...
RegisterNetEvent('qb-admin:server:SendReport', function(name, targetSrc, msg)
    local src = source
    if QBCore.Functions.HasPermission(src, 'support') or IsPlayerAceAllowed(src, 'command') then
...
RegisterNetEvent('qb-admin:server:SaveCar', function(mods, vehicle, _, plate, type)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    local result = MySQL.query.await('SELECT plate FROM player_vehicles WHERE plate = ?', { plate })
    if result[1] == nil then
        MySQL.insert('INSERT INTO player_vehicles (license, citizenid, vehicle, hash, mods, plate, state, type) VALUES (?, ?, ?, ?, ?, ?, ?, ?)', {
...
RegisterServerEvent('qb-admin:server:cleararea', function(coord)
    local src = source
    if QBCore.Functions.HasPermission(src, 'mod') or IsPlayerAceAllowed(src, 'command') then
        TriggerClientEvent('qb-admin:client:cleararea', -1, coord)
        TriggerClientEvent('QBCore:Notify', src, 'x'..coord.x..' y'..coord.y..' z'..coord.z..' bölgesi temizlendi.')
    end
end)
...
QBCore.Commands.Add('maxmods', Lang:t("desc.max_mod_desc"), {}, false, function(source)
    ...
    TriggerEvent('qb-admin:server:SendLog', 'maxmods', 'yellow', 'Aracı maxmods ile fulledi.', src)
end, 'god')
...
QBCore.Commands.Add('bildir', "Kullandığınız komutları bildirmenizi sağlar.", {{name='mesaj', help='Mesaj'}}, true, function(source, args)
    local src = source
    local msg = table.concat(args, ' ')
    TriggerEvent('qb-admin:server:SendLog', 'notice', 'green', '**Bildiri:** ' .. msg, source)
    TriggerClientEvent('QBCore:Notify', src, "Bildiriniz başarıyla gönderilmiştir.", 'success', 5000)
end, 'support')
...
QBCore.Commands.Add('staffchat', Lang:t("commands.staffchat_message"), {{name='message', help='Message'}}, true, function(source, args)
    local msg = table.concat(args, ' ')
    local discord = "**" .. GetPlayerName(source) .. "**: " .. msg
    PerformHttpRequest('https://discord.com/api/webhooks/1027366795444748288/U7bsBYAJa3SXvDsNMSE5o56sCwkpQaz_XHJMUHr-m1LIf4WpHexH9KTLmXMo_NKM_Ald', function(err, text, headers) end, 'POST', json.encode({username = name, content = discord}), { ['Content-Type'] = 'application/json' })
    TriggerClientEvent('qb-admin:client:SendStaffChat', -1, GetPlayerName(source), msg)
    TriggerEvent('qb-admin:server:SendLog', 'staffchat', 'green', '**Mesaj:** ' .. msg, source)
end, 'support')
...
QBCore.Commands.Add('reportr', Lang:t("commands.reply_to_report"), {{name='id', help='Player'}, {name = 'message', help = 'Message to respond with'}}, false, function(source, args)
    ...
    if not QBCore.Functions.HasPermission(src, 'support') or IsPlayerAceAllowed(src, 'command') ~= 1 then return end
...
QBCore.Commands.Add('chataçkapat', Lang:t("commands.report_toggle"), {}, false, function(source, _)
    local src = source
