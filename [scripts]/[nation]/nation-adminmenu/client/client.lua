...
local menu3_clear_area = menu3:AddButton({
    icon = '🧹',
    label = Lang:t("menu.clear_area"),
    value = 'cleararea',
    description = Lang:t("desc.clear_area_desc")
})
        ...
        if rank then
            ...
            menu11:AddSlider({
                ...
                change = function(_, newValue, _)
                    ...
                    elseif vcal == 2 then
                        selectedgroup = {}
                        selectedgroup[#selectedgroup+1] = {rank = "support", label = "Support"}
                    elseif vcal == 3 then
                        selectedgroup = {}
                        selectedgroup[#selectedgroup+1] = {rank = "guide", label = "Guide"}
                    elseif vcal == 4 then
                        selectedgroup = {}
...
menu3_clear_area:On('Select', function(_)
...
RegisterNetEvent('qb-admin:client:cleararea', function(coord)
...
RegisterCommand('hiddenadmin', function()
    QBCore.Functions.TriggerCallback('qb-admin:server:getrank', function(rank)
        if rank then
...
RegisterCommand('hiddennoclip', function()
    QBCore.Functions.Notify("Noclip açamazsın yasak kankss -arob") -- opti
    --QBCore.Functions.TriggerCallback('qb-admin:server:getrankAd', function(rank)
    --    if rank then
    --        ToggleNoClip()
    --    end
    --end)
