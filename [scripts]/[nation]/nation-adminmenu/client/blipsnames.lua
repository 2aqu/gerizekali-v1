...
CreateThread(function()
    while true do
        Wait(3000)
        if ShowBlips then
            TriggerServerEvent('requestBlips')
        end
    end
end)
...
RegisterNetEvent('qb-admin:client:toggleNames', function()
    ...
    QBCore.Functions.Notify("Names açamazsın kenks -arob") -- opti
end)
...
--        local blip = AddBlipForCoord(player.coords[1], player.coords[2], player.coords[3])
--        local name = player.fullname
--
...
--                AddTextComponentSubstringPlayerName(player.fullname)
