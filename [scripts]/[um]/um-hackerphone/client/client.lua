	...
	if dist < 2 then 
		...
		exports['ps-ui']:Scrambler(function(success)
			if success then
				...
				ClearPedTasks(ped)
				QBCore.Functions.Notify('Merkezi araba bağlı', "success")
				TriggerServerEvent('um-hackerphone:server:removeitem',"centralchip")
			else
				ClearPedTasks(ped)
				QBCore.Functions.Notify('Bağlanılamadı', "error")
			end
		end, "numeric", 30, 2)
	else
		QBCore.Functions.Notify('Elektrik santrali yakınında değil', "error")
	end
end)
		...
		if #(pos - vehpos) < 2 then
			...
			exports['ps-ui']:Circle(function(success)
				if success then
					...
					ClearPedTasks(ped)
					QBCore.Functions.Notify('Araca bağlı takip cihazı', "success")
			...
			QBCore.Functions.Notify('Yakınlarda araba yok', "error")
		end
    end
end)
