...
local function TakeOutVehicle(vehicleInfo)
    ...
    if coords then
        ...
        QBCore.Functions.TriggerCallback('police:server:PayForVehicle', function(result)
            if not result then
                return QBCore.Functions.Notify(Lang:t('error.not_enough_money'), 'error', 4500)
            end
            QBCore.Functions.TriggerCallback('QBCore:Server:SpawnVehicle', function(netId)
                local veh = NetToVeh(netId)
                SetCarItemsInfo()
...
local function SetWeaponSeries()
    for k, _ in pairs(Config.Items.items) do
        if k < 6 then
            Config.Items.items[k].info.serie = tostring(QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) ..
        ...
        if result == nil then
            ...
            for _, v in pairs(result) do
                ...
                impoundMenu[#impoundMenu + 1] = {
                    ...
                    optdesc = Lang:t('info.vehicle_info', {
                        billvalue = v.depotprice,
                        value = enginePercent,
                        value2 = currentFuel
                    }),
                    optparams = {
...
RegisterNetEvent('police:client:ImpoundVehicle', function(fullImpound)
    ...
    if dialog then
        ...
        if tonumber(dialog.price) < 0 then
            price = 0
        else
            price = dialog.price
        end
        if vehicle ~= 0 and vehicle then
            ...
            if #(pos - vehpos) < 5.0 and not IsPedInAnyVehicle(ped) then
                QBCore.Functions.Progressbar('impound', Lang:t('progressbar.impound'), 5000, false, true, {
                    ...
                    TriggerServerEvent("police:server:Impound", plate, fullImpound, price, bodyDamage, engineDamage,
                        totalFuel)
...
RegisterNetEvent('police:client:VehicleSubMenu', function(data)
    ...
    if data.out then
        SubMenu[#SubMenu + 1] = {
            opthead = 'Return ' .. vehLabel,
            optdesc = "",
            opticon = 'car',
            optparams = {
                event = "police:client:ReturnVehicle",
                args = {
                    car = data.car
                }
            }
        ...
        SubMenu[#SubMenu + 1] = {
            opthead = 'Take out ' .. vehLabel,
            optdesc = "Take out for $" .. data.price,
            opticon = 'car',
            optparams = {
    ...
    if #(pos - takeLoc) <= 1.0 then
        if currentType == 'drawer' then
            ...
            if drawer then
                ...
                TriggerEvent("inventory:client:SetCurrentStash", Lang:t('info.current_evidence', {
                    value = currentEvidence,
                    value2 = drawer.slot
                }))
            else
        ...
        elseif currentType == 'stash' then
            TriggerServerEvent("inventory:server:OpenInventory", "stash", Lang:t('info.general_current_evidence', {
                value = currentEvidence
            }), {
                maxweight = 4000000,
                slots = 300
            })
            TriggerEvent("inventory:client:SetCurrentStash", Lang:t('info.general_current_evidence', {
                value = currentEvidence
            }))
        end
...
RegisterNetEvent('qb-policejob:ToggleDuty', function()
    ...
    if PlayerJob.onduty then
        dutystatus = '🟢 ' .. Lang:t('menu.dty_onduty')
    else
        dutystatus = '🔴 ' .. Lang:t('menu.dty_offduty')
    end
    ...
    local header = {
        ...
        desc = 'Mesai durumunuz: ' .. dutystatus,
        icon = "retweet"
    }
    if PlayerJob.onduty then
        dutymenu[#dutymenu + 1] = {
            opthead = Lang:t('menu.dty_beonduty'),
            optdesc = '',
            opticon = 'signature',
            optdisabled = true
        }
        dutymenu[#dutymenu + 1] = {
            opthead = Lang:t('menu.dty_beoffduty'),
            optdesc = '',
            opticon = 'signature',
            optparams = {
                isServer = true,
                event = 'police:server:changeDuty',
                args = {
                    duty = false
                }
            }
...
RegisterNetEvent('qb-police:client:spawnHelicopter', function(k)
    if IsPedInAnyVehicle(PlayerPedId(), false) then
        ...
        if Heli then
            QBCore.Functions.Notify(Lang:t("error.alradyhaveheli"), "error", 4500)
            return
        end
        if IsAnyVehicleNearPoint(coords.x, coords.y, coords.z, 2.0) then
            QBCore.Functions.Notify(Lang:t("error.clearspawnpoint"), "error", 4500)
...
RegisterNetEvent('policejob:client:VehicleLiveryMenu', function(data)
    ...
    for k, v in pairs(liveries) do
        LiveryMenu[#LiveryMenu + 1] = {
            opthead = v.name,
            optdesc = 'Change livery to ' .. v.name,
            opticon = 'brush',
            optparams = {
                event = 'police:client:ChangeLivery',
                args = {
                    id = v.id
                }
            }
...
RegisterNetEvent('policejob:client:VehicleExtrasMenu', function(data)
    ...
    for k, v in pairs(extras) do
        ExtraMenu[#ExtraMenu + 1] = {
            opthead = 'Extra ' .. v.id,
            optdesc = 'Change extra option ' .. v.id,
            opticon = 'caret-right',
            optparams = {
                event = 'police:client:ChangeExtra',
                args = {
                    id = v.id,
                    veh = vehicle
                }
...
RegisterNetEvent('police:client:TowMenu', function(Vehicle, Plate)
    ...
    local TowMenu = {}
    TowMenu[#TowMenu + 1] = {
        opthead = Lang:t('menu.tow_menu_depot_h'),
        optdesc = Lang:t('menu.tow_menu_depot_b'),
        opticon = 'truck-pickup',
        optparams = {
    ...
    TowMenu[#TowMenu + 1] = {
        opthead = Lang:t('menu.tow_menu_impound_h'),
        optdesc = Lang:t('menu.tow_menu_impound_b'),
        opticon = 'shield',
        optparams = {
            event = 'police:client:Tow',
            args = {
    ...
    local header = {
        ...
        desc = Lang:t('menu.tow_menu_header_b', {
            vehicle = ModelBrand .. ' ' .. ModelName,
            plate = Plate
        }),
        icon = "car-tunnel"
    }
                ...
                if IsPedInAnyVehicle(PlayerPedId(), false) then
                    if IsControlJustReleased(0, 38) then
                        ...
                        if dialog then
                            ...
                            Wait(1000)
                            TriggerServerEvent("police:server:Impound", Plate, true, price, bodyDamage, engineDamage,
                                totalFuel)
                            while NetworkGetEntityOwner(Vehicle) ~= 128 do -- Ensure we have entity ownership to prevent inconsistent vehicle deletion
                                NetworkRequestControlOfEntity(Vehicle)
                                Wait(100)
...
if Config.UseTarget then
                ...
                Options = {{
                    ...
                    action = function()
                        ...
                        EvidenceStash[#EvidenceStash + 1] = {
                            opthead = Lang:t('menu.evd_drawer_h'),
                            optdesc = Lang:t('menu.evd_drawer_b'),
                            opticon = 'list-ol',
                            optparams = {
                        ...
                        EvidenceStash[#EvidenceStash + 1] = {
                            opthead = Lang:t('menu.evd_stash_h'),
                            optdesc = Lang:t('menu.evd_stash_b'),
                            opticon = 'folder-closed',
                            optparams = {
                                event = 'police:client:EvidenceStashDrawer',
                                args = {
                                    type = 'stash',
        ...
        for k, v in pairs(Config.Locations["helicopter"]) do
            ...
            AddBoxZone("HeliPed_" .. k, {
                ...
                Options = {{
                    ...
                    canInteract = function()
                        if PlayerJob.type == "leo" and not Heli then
                            return true
                        end
                    end,
                    action = function()
                    ...
                    canInteract = function()
                        if PlayerJob.type == "leo" and Heli then
                            return true
                        end
                    end,
                    action = function()
    ...
    for _, v in pairs(Config.Locations["duty"]) do
        dutyZones[#dutyZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 1.75, 1, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    for _, v in pairs(Config.Locations["stash"]) do
        stashZones[#stashZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 1.5, 1.5, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    for _, v in pairs(Config.Locations["trash"]) do
        trashZones[#trashZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 1, 1.75, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    local trashCombo = ComboZone:Create(trashZones, {
        name = "trashCombo",
        debugPoly = false
    })
    ...
    for _, v in pairs(Config.Locations["fingerprint"]) do
        fingerprintZones[#fingerprintZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 2, 1, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    for _, v in pairs(Config.Locations["armory"]) do
        armouryZones[#armouryZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 5, 1, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    for _, v in pairs(Config.Locations["evidence"]) do
        evidenceZones[#evidenceZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 2, 1, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    for _, v in pairs(Config.Locations["vehicle"]) do
        garageZones[#garageZones + 1] = BoxZone:Create(vector3(v.x, v.y, v.z), 3, 3, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1
    ...
    for k, v in pairs(Config.Locations["helispawn"]) do
        helicopterZones[#helicopterZones + 1] = BoxZone:Create(vector3(vector3(v.x, v.y, v.z)), 10, 10, {
            name = "box_zone",
            data = {
                zone = k
            },
...
CreateThread(function()
    ...
    for _, v in pairs(Config.Locations["impound"]) do
        impoundZones[#impoundZones + 1] = BoxZone:Create(vector3(v.x, v.y, v.z), 4, 4, {
            name = "box_zone",
            debugPoly = false,
            minZ = v.z - 1,
            maxZ = v.z + 1,
...
RegisterCommand('liverymenu', function()
    if not PlayerJob.type == "leo" then
        QBCore.Functions.Notify("You can't use this menu..", "error")
        return
    end
    local vehicle = nil
    if IsPedInAnyVehicle(PlayerPedId(), false) then
    ...
    LiveryMenu = {}
    LiveryMenu[#LiveryMenu + 1] = {
        opthead = "Change Liveries",
        optdesc = "Change your vehicle liveries",
        opticon = 'brush',
        optparams = {
            event = 'policejob:client:VehicleLiveryMenu',
            args = {
                vehicle = vehicle
            }
        }
    ...
    LiveryMenu[#LiveryMenu + 1] = {
        opthead = "Change Extras",
        optdesc = "Change your vehicle extras",
        opticon = 'brush',
        optparams = {
            event = 'policejob:client:VehicleExtrasMenu',
            args = {
                vehicle = vehicle
            }
        }
