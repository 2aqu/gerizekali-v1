...
-- QBCore.Commands.Add("taşı", Lang:t("commands.escort"), {}, false, function(source)
--     local src = source
...
QBCore.Commands.Add("mtemizle", Lang:t("commands.clear_casign"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("ktemizle", Lang:t("commands.clearblood"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("parayial", Lang:t("commands.seizecash"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("kcek", Lang:t("commands.impound"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("ekelepce", Lang:t("commands.anklet"), {}, false, function(source)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("ekelepcetakip", Lang:t("commands.ankletlocation"), {{name = "cid", help = Lang:t('info.citizen_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
    if Player.PlayerData.job.type == "leo" and Player.PlayerData.job.onduty then
...
QBCore.Commands.Add("dnaal", Lang:t("commands.takedna"), {{name = "id", help = Lang:t('info.player_id')}}, true, function(source, args)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
...
for _,v in pairs(Config.CuffItems) do
    QBCore.Functions.CreateUseableItem(v.itemname , function(source,item)
        ...
        TriggerClientEvent('QBCore:Notify', source, "Bu kelepçe eski bir eşya. Yenilerini kullanın.", "error")
    end)
end
...
QBCore.Functions.CreateCallback('police:GetAllCops', function(_, cb)
    local allcops = {}
    local players = QBCore.Functions.GetQBPlayers()
    for _, v in pairs(players) do
        if v and v.PlayerData.job.type == "leo" then
            ...
            table.insert(allcops, {fullname = "[" .. _ .. "] " .. dutyText .. v.PlayerData.charinfo.firstname .. " " .. v.PlayerData.charinfo.lastname .. " | " .. v.PlayerData.job.name })
        end
    end
    cb(allcops)
end)
...
QBCore.Functions.CreateCallback('police:server:GetEvidenceByType', function(source, cb, type)
    local Player = QBCore.Functions.GetPlayer(source)
    local EvidenceBags = exports[Config.Inventory]:GetSlotsWithItem(source, 'filled_evidence_bag')
    QBCore.Debug(EvidenceBags)
...
QBCore.Functions.CreateCallback('coast-getpolice', function(_, cb)
    cb(GetCurrentCops())
end)