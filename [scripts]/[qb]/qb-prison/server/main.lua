...
RegisterNetEvent('prison:server:GiveJailItems', function(escaped)
    ...
    for _, v in pairs(Player.PlayerData.metadata["jailitems"]) do
        Player.Functions.AddItem(v.name, v.amount or v.count, false, v.info)
    end
    Player.Functions.SetMetaData("jailitems", {})
end)
...
RegisterNetEvent('prison:server:SecurityLockdown', function()
    ...
    for _, v in pairs(QBCore.Functions.GetPlayers()) do
        ...
        if Player then
            if (Player.PlayerData.job.name == "police" or Player.PlayerData.job.name == "fbi" or Player.PlayerData.job.name == "state" or Player.PlayerData.job.name == "sheriff" or Player.PlayerData.job.name == "ranger" or Player.PlayerData.job.name == "davison") and Player.PlayerData.job.onduty then
...
RegisterNetEvent('prison:server:JailAlarm', function()
    ...
    if #(coords.xy - middle) < 200 then return end
