...
CreateThread(function()
    ...
    while true do
        ...
        if LocalPlayer.state.isLoggedIn then
            if (PlayerJob.name ~= "police" or PlayerJob.name ~= "fbi" or PlayerJob.name ~= "state" or PlayerJob.name ~= "sheriff" or PlayerJob.name ~= "ranger" or PlayerJob.name ~= "davison") then
                ...
                for k in pairs(Gates) do
                    ...
                    if dist < 1.5 then
                        ...
                        if securityLockdown then
                            sleep = 0
                            DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "~r~SISTEM KITLENDI")
                        elseif Gates[k].hit then
                            sleep = 0
                            DrawText3D(Gates[k].coords.x, Gates[k].coords.y, Gates[k].coords.z, "SISTEM IHLALI")
