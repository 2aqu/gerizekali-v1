			...
			if not success then
				...
				if closestZone then
					...
					if next(nuiData) then
						...
						SendNUIMessage({response = "foundTarget", data = nuiData[slot].targeticon})
						if Config.DrawSprite then
							if listSprite and closestZone then
								if closestZone.name then
									if listSprite[closestZone.name] then
...
RegisterNetEvent("arob:detelecar")
AddEventHandler("arob:detelecar", function()
    local ped = PlayerPedId()
    local pedCoords = GetEntityCoords(ped)
    local maxDistance = 10.0 -- Silme mesafesini burada ayarlayabilirsiniz

    local vehicle = GetClosestVehicle(pedCoords.x, pedCoords.y, pedCoords.z, maxDistance, 0, 71) -- 71 araç hash'i, dilediğiniz araç hash'ini kullanabilirsiniz.
