...
RegisterNetEvent('qb-garage:server:updateVehicle', function(state, mods, fuel, engine, body, plate, garage)
    MySQL.Async.execute('UPDATE player_vehicles SET state = ?, mods = ?, garage = ?, fuel = ?, engine = ?, body = ? WHERE plate = ?',{state,json.encode(mods), garage, fuel, engine, body, plate})
end)
