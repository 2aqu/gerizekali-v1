    ...
    QBCore.Functions.TriggerCallback('qb-garage:server:checkOwnership', function(owned)
        ...
        if owned then
            ...
            if FuelScript then
                ...
                totalFuel = exports['cdn-fuel']:GetFuel(veh) -- Don't change this. Change it in the config. Defaults to legacy fuel if not set in the config
            end

            if not CanParkVehicle(veh) then return end
            local myCar = QBCore.Functions.GetVehicleProperties(veh)
            TriggerServerEvent('qb-garage:server:updateVehicle', 1, myCar, totalFuel, engineDamage, bodyDamage, plate, garage)
...
local function CreateGarageZone(zone, garageName)
    zone:onPlayerInOut(function(isPointInside)
        if isPointInside then
            ...
            exports['qb-radialmenu']:RemoveOption(7)
...
RegisterNetEvent('qb-garages:client:TakeOutGarage', function(data, cb)
    ...
    QBCore.Functions.TriggerCallback('arob:billcheck', function(statusInfo)
        if statusInfo then
            local totalAmount = tonumber(statusInfo.totalAmount)
            local paidCount = tonumber(statusInfo.paidCount)
            local unpaidCount = tonumber(statusInfo.unpaidCount)
            ...
            if unpaidCount == 0 then
                QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                    QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
                        ...
                        if FuelScript then
                            ...
                            exports['cdn-fuel']:SetFuel(veh, vehicle.fuel) -- Don't change this. Change it in the config. Defaults to legacy fuel if not set in the config
                        end
                ...
                QBCore.Functions.Notify('Ödenmemiş faturanız bulunmakta.', 'error')
            end
        else
            QBCore.Functions.SpawnVehicle(vehicle.vehicle, function(veh)
                QBCore.Functions.TriggerCallback('qb-garage:server:GetVehicleProperties', function(properties)
