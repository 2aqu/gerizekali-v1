...
function QBCore.Player.Login(source, citizenid, newData)
    if source and source ~= '' then
        if citizenid then
            ...
            if PlayerData and license == PlayerData.license then
                ...
                TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white',
                    GetPlayerName(source) .. ' karakterindeki sorun sebebiyle sunucudan atıldı.', false)
            end
        else
        ...
        QBCore.ShowError(GetCurrentResourceName(), 'ERROR QBCORE.PLAYER.LOGIN - oyuncu kimliği bulunamadı!')
        return false
    end
end
...
function QBCore.Player.CheckPlayerData(source, PlayerData)
    ...
    PlayerData.metadata['bloodtype'] = PlayerData.metadata['bloodtype'] or
                                           QBCore.Config.Player.Bloodtypes[math.random(1,
    ...
    PlayerData.items = GetResourceState('qb-inventory') ~= 'missing' and
                           exports['qb-inventory']:LoadInventory(PlayerData.source, PlayerData.citizenid) or {}
...
function QBCore.Player.CreatePlayer(PlayerData, Offline)
    ...
    function self.Functions.AddMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            ...
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Para Verildi', 'lightgreen',
                    '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
                        ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype ..
                        ') eklendi, yeni ' .. moneytype .. ' miktarı: ' .. self.PlayerData.money[moneytype] ..
                        ' sebep: ' .. reason, true)
            else
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Para Verildi', 'lightgreen',
                    '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
    ...
    function self.Functions.RemoveMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            if amount > 100000 then
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Para Alındı', 'red',
                    '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
                        ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype ..
                        ') alındı, yeni ' .. moneytype .. ' miktarı: ' .. self.PlayerData.money[moneytype] ..
                        ' sebep: ' .. reason, true)
            else
                TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Para Alındı', 'red',
                    '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
    ...
    function self.Functions.SetMoney(moneytype, amount, reason)
        ...
        if not self.Offline then
            self.Functions.UpdatePlayerData()
            TriggerEvent('qb-log:server:CreateLog', 'playermoney', 'Para Belirlendi', 'green',
                '**' .. GetPlayerName(self.PlayerData.source) .. ' (citizenid: ' .. self.PlayerData.citizenid ..
                    ' | id: ' .. self.PlayerData.source .. ')** $' .. amount .. ' (' .. moneytype ..
                    ') belirlendi, yeni ' .. moneytype .. ' miktarı: ' .. self.PlayerData.money[moneytype] ..
                    ' sebep: ' .. reason)
...
--                   end
--                   QBCore.ShowSuccess(GetCurrentResourceName(), PlayerData.name .. ' oyuncu verileri başarıyla kaydedildi!')
--               end)
--           return
--       end)
--
--   else
--       QBCore.ShowError(GetCurrentResourceName(),
--           'ERROR QBCORE.PLAYER.SAVE - veriler boş olduğu için kaydedilemedi!')
--   end
--end
 ...
 function QBCore.Player.Save(source)
     ...
     local PlayerData = QBCore.Players[source].PlayerData
     if PlayerData then
         if saveSql(PlayerData, pcoords) then
            if GetResourceState('qb-inventory') ~= 'missing' then
                exports['qb-inventory']:SaveInventory(source)
            end
         end
 ...
 function saveSql(PlayerData, pcoords)
     local p = promise:new()
...
function QBCore.Player.SaveOffline(PlayerData)
    if PlayerData then
        ...
        QBCore.ShowSuccess(GetCurrentResourceName(),
            PlayerData.name .. ' pasif oyuncunun verileri başarıyla kaydedildi!')
    else
        QBCore.ShowError(GetCurrentResourceName(),
            'ERROR QBCORE.PLAYER.SAVEOFFLINE - veriler boş olduğu için kaydedilemedi!')
    end
end
...
function QBCore.Player.DeleteCharacter(source, citizenid)
    ...
    if license == result then
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('qb-log:server:CreateLog', 'joinleave', 'Karakter Silindi', 'red',
                    '**' .. GetPlayerName(source) .. '** ' .. license .. 'silindi **' .. citizenid .. '**..')
            end
        end)
    else
        DropPlayer(source, Lang:t("info.exploit_dropped"))
        TriggerEvent('qb-log:server:CreateLog', 'anticheat', 'Anti-Cheat', 'white',
            GetPlayerName(source) .. ' karakter silme hilesi sebebiyle sunucudan atıldı.', true)
    end
end
...
function QBCore.Player.ForceDeleteCharacter(citizenid)
    ...
    if result then
        ...
        if Player then
            DropPlayer(Player.PlayerData.source, "Bir yönetici şu anda kullanmakta olduğunuz karakteri sildi.")
        end
        for i = 1, tableCount do
            local v = playertables[i]
            queries[i] = {
        ...
        MySQL.transaction(queries, function(result2)
            if result2 then
                TriggerEvent('qb-log:server:CreateLog', 'joinleave', 'Karakter Yönetici Tarafından Silindi', 'red',
                    'Karakter **' .. citizenid .. '** başarıyla silindi.')
            end
        end)
    end
end
...
function QBCore.Functions.CreateAccountNumber()
    ...
    while not UniqueFound do
        AccountNumber = 'WA0' .. math.random(1, 9) .. math.random(1111, 9999) .. math.random(1111, 9999) ..
                            math.random(11, 99)
...
function QBCore.Player.CreateFingerId()
    ...
    local FingerId = nil
    while not UniqueFound do
        FingerId = tostring(QBCore.Shared.RandomStr(2) .. QBCore.Shared.RandomInt(3) .. QBCore.Shared.RandomStr(1) ..
                                QBCore.Shared.RandomInt(2) .. QBCore.Shared.RandomStr(3) .. QBCore.Shared.RandomInt(4))
