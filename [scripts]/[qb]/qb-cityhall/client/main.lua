                    ...
                    if isLoggedIn and closestCityhall and closestDrivingSchool then
                        if inside then
                            if current.drivingschool then
                                ...
                                exports['qb-core']:DrawText('[E] Sürüş Dersleri Alın')
                            elseif current.cityhall then
                                inRangeCityhall = true
                                exports['qb-core']:DrawText('[E] Şehir Hizmetleri')
                            end
                        else
...
RegisterNUICallback('close', function(_, cb)
    ...
    if not Config.UseTarget and inRangeCityhall then exports['qb-core']:DrawText('[E] Şehir Hizmetleri') end -- Reopen interaction when you're still inside the zone
    cb('ok')
end)
...
RegisterNUICallback('requestId', function(id, cb)
    ...
    if inRangeCityhall and license and id.cost == license.cost then
        ...
        QBCore.Functions.Notify(('$%s karşılığında %s alındı.'):format(id.cost, license.label), 'success', 3500)
    else
