...
AddEventHandler('gameEventTriggered', function(event, data)
    if event == "CEventNetworkEntityDamage" then
        ...
        if victimDied and NetworkGetPlayerIndexFromPed(victim) == PlayerId() and IsEntityDead(PlayerPedId()) then
            ...
            TriggerServerEvent("hospital:server:SendLog", GetPlayerServerId(playerid), GetPlayerServerId(killerId), weaponName, weaponLabel)
            --TriggerServerEvent("qb-log:server:CreateLog", "death", Lang:t('logs.death_log_title', {playername = playerName}), "red", Lang:t('logs.death_log_message', {killername = killerName, playername = playerName, weaponlabel = weaponLabel, weaponname = weaponName}))
        end
    end
end)
    ...
    if event == "CEventNetworkEntityDamage" then
        if data[6] == 1 then
            ...
            if GetPlayerServerId(killerId) == GetPlayerServerId(PlayerId()) then
                exports['screenshot-basic']:requestScreenshotUpload("https://discord.com/api/webhooks/1052345584390000770/RsCNUvYJ5bome6dMZANymCJkX7RGiLoeQZKnrqq73u16L7xwZi3bjtFDoxe6CUsyRbf-", 'files[]', {encoding = 'jpg'}, function(data)
                    local resp = json.decode(data)
                    TriggerServerEvent('qb-log:server:CreateLog', "death", "Foto Öldürme Log", "default", resp.attachments[1].url, false, GetPlayerServerId(PlayerId()))
                end)
            end
        end
        local victim, attacker, victimDied, weapon = data[1], data[2], data[4], data[7]
        ...
        if victimDied and NetworkGetPlayerIndexFromPed(victim) == PlayerId() and IsEntityDead(PlayerPedId()) then
            if not InLaststand then
                ...
                if weapon == -842959696 or weapon == -1569615261 or weapon == -544306709 or weapon == -10959621 or weapon == -1553120962 or weapon == -1024456158 then
                    ...
                    local killerId = NetworkGetPlayerIndexFromPed(attacker)
                    local killerName = GetPlayerName(killerId) .. " " .. "("..GetPlayerServerId(killerId)..")" or Lang:t('info.self_death')
                    local weaponlabel = 'Bilinmiyor'
                    local weaponName = 'Bilinmiyor'
                    if QBCore.Shared.Weapons[weapon] then
                        weaponLabel = QBCore.Shared.Weapons[weapon].label or 'Bilinmiyor'
                        weaponName = QBCore.Shared.Weapons[weapon].name or 'Bilinmiyor'
                    end
        ...
        if player ~= -1 and distance < 2.5 then
            ...
            if IsPedInAnyVehicle(PlayerPedId()) then
                ...
                if IsEntityPlayingAnim(playerped, 'combat@damage@writhe', 'writhe_loop', 3) then
                    sleepaq = 1
                    kordinat = GetEntityCoords(playerped)
                    QBCore.Functions.DrawText3D(kordinat.x , kordinat.y, kordinat.z, "[E] Kisiye Yardım Et")
                    if IsControlJustPressed(0, 38) then
                        TriggerEvent('hospital:client:UseFirstAid')
                    end
                else
                    sleepaq = 1000
                end
