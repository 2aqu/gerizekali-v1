...
local function LeaveBed()
	...
	if PlayerData.metadata["injail"] > 999 then
...
RegisterNetEvent('hospital:client:adminHeal', function()
    ...
    -- TriggerServerEvent("QBCore:Server:SetMetaData", "thirst", 50)
    TriggerServerEvent("consumables:server:addThirst", QBCore.Functions.GetPlayerData().metadata["hunger"] + 50)
    TriggerServerEvent("consumables:server:addHunger", QBCore.Functions.GetPlayerData().metadata["thirst"] + 50)
end)
...
RegisterNetEvent('qb-ambulancejob:checkin', function()
    QBCore.Functions.TriggerCallback('arob:server:doctorcheck', function(doctor)
        if doctor <= 1 then
            ...
            QBCore.Functions.Notify("Mesaide doktor var yardımcı olamam..", "error")
        end
    end)
end)
...
--[[         for k, v in pairs(Config.Locations["beds"]) do
            exports['qb-target']:AddBoxZone("beds"..k,  v.coords, 2.5, 2.3, {
                ...
                options = {
                    {
                        type = "client",
                        event = "qb-ambulancejob:beds:target",
                        icon = "fas fa-bed",
                        label = "Yatağa Yat",
                    }
                },
