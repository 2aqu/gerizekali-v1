...
RegisterNetEvent('hospital:server:SendLog', function(x, y, weaponName, weaponLabel)
	if x ~= 0 and y ~= 0 then
		local xPlayer = QBCore.Functions.GetPlayer(x)
		local yPlayer = QBCore.Functions.GetPlayer(y)
		TriggerEvent('qb-log:server:CreateLog', 'death', 'Ölüm ('..xPlayer.PlayerData.name..')', "lightgreen", "Ölen: "..xPlayer.PlayerData.name .. " (CitizenID: "..xPlayer.PlayerData.citizenid.." | Isim: ".. xPlayer.PlayerData.charinfo.firstname .. " " .. xPlayer.PlayerData.charinfo.lastname .." | ID: "..xPlayer.PlayerData.source..")\nÖldüren: "..yPlayer.PlayerData.name .. " (CitizenID: "..yPlayer.PlayerData.citizenid.." | Isim: ".. yPlayer.PlayerData.charinfo.firstname .. " " .. yPlayer.PlayerData.charinfo.lastname .." | ID: "..yPlayer.PlayerData.source..")\nSilah İsmi: ".. weaponName .."\nSilah Açıklaması: ".. weaponLabel)
	elseif y == 0 and x ~= 0 then
		local xPlayer = QBCore.Functions.GetPlayer(x)
		TriggerEvent('qb-log:server:CreateLog', 'death', 'Ölüm ('..xPlayer.PlayerData.name..')', "orange", "Ölen: "..xPlayer.PlayerData.name .. " (CitizenID: "..xPlayer.PlayerData.citizenid.." | Isim: ".. xPlayer.PlayerData.charinfo.firstname .. " " .. xPlayer.PlayerData.charinfo.lastname .." | ID: "..xPlayer.PlayerData.source..")\nÖldüren: NPC veya Kendisi\nSilah İsmi: ".. weaponName .."\nSilah Açıklaması: ".. weaponLabel)
	end
end)
...
RegisterNetEvent('hospital:server:RespawnAtHospital', function()
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player.PlayerData.metadata["injail"] > 999 then
...
RegisterNetEvent('hospital:server:removeDoping', function()
	local Player = QBCore.Functions.GetPlayer(source)

	if not Player then return end

	Player.Functions.RemoveItem('doping', 1)
end)
...
RegisterNetEvent('hospital:server:resetHungerThirst', function()
	local Player = QBCore.Functions.GetPlayer(source)
	if Player.PlayerData.metadata["hunger"] < 20 then
		Player.Functions.SetMetaData('hunger', 40)
		TriggerClientEvent('hud:client:UpdateNeeds', source, 20, Player.PlayerData.metadata["thirst"])
	end
	if Player.PlayerData.metadata["thirst"] < 20 then
		Player.Functions.SetMetaData('thirst', 40)
		TriggerClientEvent('hud:client:UpdateNeeds', source, Player.PlayerData.metadata["hunger"], 20)
	end
	if not Player then return end
end)
...
QBCore.Commands.Add("revive", Lang:t('info.revive_player_a'), {{name = "id", help = Lang:t('info.player_id')}}, false, function(source, args)
	...
	if args[1] then
		...
		if Player then
			...
			TriggerEvent('qb-admin:server:SendServerLog', source, Player.PlayerData.source, 'revive', 'green', 'isimli oyuncuya revive attı.')
		else
		...
		TriggerEvent('qb-admin:server:SendLog', 'revive', 'yellow', 'Kendine revive attı.', source)
	end
end, "guide")
...
QBCore.Commands.Add('arobheal', 'Bir Oyuncuyu veya Kendinizi İyileştirin (Admin Only)', {{name='id', help='Player ID (may be empty)'}}, false, function(source, args)
	local src = source
	if args[1] then
		local Player = QBCore.Functions.GetPlayer(tonumber(args[1]))
		if Player then
...
QBCore.Functions.CreateUseableItem("doping", function(source, item)
	local src = source
	local Player = QBCore.Functions.GetPlayer(src)
	if Player.Functions.GetItemByName(item.name) ~= nil then
		TriggerClientEvent("hospital:client:UseDoping", src)
	end
end)
...
QBCore.Functions.CreateCallback('arob:server:doctorcheck', function(source, cb)
	local amount = 0
