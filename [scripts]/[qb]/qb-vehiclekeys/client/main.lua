...
RegisterKeyMapping('engine', Lang:t("info.engine"), 'MOUSE_BUTTON', 'MOUSE_MIDDLE')
...
RegisterNetEvent('qb-vehiclekeys:client:ToggleEngine', function()
    ...
    if HasKeys(QBCore.Functions.GetPlate(vehicle)) then
        if EngineOn then
            SetVehicleEngineOn(vehicle, false, false, true)
            QBCore.Functions.Notify("Motor kapatıldı.")
        else
            SetVehicleEngineOn(vehicle, true, false, true)
            QBCore.Functions.Notify("Motor çalıştırıldı.")
        end
    end
end)
...
function Hotwire(vehicle, plate)
    ...
    end, math.floor(hotwireTime / 3000), 8) -- NumberOfCircles, MS
end
...
function CarjackVehicle(target)
    ...
    QBCore.Functions.Progressbar("rob_keys", Lang:t("progress.acjack"), Config.CarjackingTime, false, true, {}, {}, {}, {}, function()
        ...
        if hasWeapon and isCarjacking then
            ...
            isCarjacking = false
            Wait(2000)
            AttemptPoliceAlert("Araba Hırsızlığı")
...
function AttemptPoliceAlert(type)
    if not AlertSend then
        ...
        if math.random() <= chance then
           ...
           TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", Lang:t("info.palert") .. type, false)
        end
        AlertSend = true
