...
RegisterNetEvent('qb-carwash:client:washCar', function()
    ...
    washingVehicle = true
    QBCore.Functions.Progressbar("search_cabin", "Araç yıkanıyor...", math.random(4000, 8000), false, true, {
        ...
        QBCore.Functions.Notify("Yıkama iptal edildi...", "error")
        washingVehicle = false
    end)
end)
            ...
            for k in pairs(Config.CarWash) do
                ...
                if dist <= 7.5 and Driver == PlayerPed then
                    ...
                    if not washingVehicle then
                        DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], '~g~E~w~ - Arac Yıkama ($'..Config.DefaultPrice..')')
                        if IsControlJustPressed(0, 38) then
                            if dirtLevel > Config.DirtLevel then
                                TriggerServerEvent('qb-carwash:server:washCar')
                            else
                                QBCore.Functions.Notify("Araç kirli değil.", 'error')
                            end
                        end
                    else
                        DrawText3Ds(Config.CarWash[k]["coords"]["x"], Config.CarWash[k]["coords"]["y"], Config.CarWash[k]["coords"]["z"], 'Bu araç yıkama uygun değil.')
                    end
                end
            end
        end
...
if Config.Aracyikama == true then
CreateThread(function()
    for k in pairs(Config.CarWash) do
