...
function InitiateMenus(isMotorcycle, vehicleHealth, categories, welcomeLabel)
    ...
    if vehicleHealth < 1000.0 and categories.repair then
        ...
        populateMenu("repairMenu", -1, "Tamir", "$" .. repairCost)
        finishPopulatingMenu("repairMenu")
    end

    --#[Main Menu]#--
    createMenu("mainMenu", welcomeLabel, "Kategori Seç")
    ...
    for k, v in ipairs(vehicleCustomisation) do
        ...
        if amountValidMods > 0 or v.id == 18 then
                ...
                for m, n in pairs(validMods) do
                    ...
                    if maxVehiclePerformanceUpgrades == 0 then
                        ...
                        if currentMod == n.id then
                            updateItem2Text(v.category:gsub("%s+", "") .. "Menu", n.id, "Yüklendi")
                        end
                    else
            ...
            elseif v.id == 18 then
                ...
                updateItem2Text(v.category:gsub("%s+", "") .. "Menu", currentTurboState, "Yüklendi")

                finishPopulatingMenu(v.category:gsub("%s+", "") .. "Menu")
            else
                createMenu(v.category:gsub("%s+", "") .. "Menu", v.category .. " Customisation", "Mod Seç")
    ...
    populateMenu("ResprayMenu", 0, "Ana Renk", "none")
    populateMenu("ResprayMenu", 1, "İkinci Renk", "none")
    populateMenu("ResprayMenu", 2, "Sedef Renk", "none")
    populateMenu("ResprayMenu", 3, "Teker Renk", "none")
    populateMenu("ResprayMenu", 4, "İç Renk", "none")
    populateMenu("ResprayMenu", 5, "Gövde Renk", "none")
    ...
    for k, v in ipairs(vehicleWheelOptions) do
        if v.id == -1 then
            ...
            print(vehicleCustomisationPrices.customwheels.price)
            updateItem2Text(v.category:gsub("%s+", "") .. "Menu", currentCustomWheelState, "Yüklendi")
    ...
    for k, v in ipairs(vehicleTyreSmokeOptions) do
        ...
        if v.r == currentWheelSmokeR and v.g == currentWheelSmokeG and v.b == currentWheelSmokeB then
            updateItem2Text("TyreSmokeMenu", k, "Yüklendi")
        end
    end
    ...
    for k, v in ipairs(vehicleWindowTintOptions) do
        ...
        if currentWindowTint == v.id then
            updateItem2Text("WindowTintMenu", v.id, "Yüklendi")
        end
    end
    ...
    if livCount > 0 then
        ...
        for i=0, livCount-1 do
            ...
            if tempOldLivery == i then
                updateItem2Text("OldLiveryMenu", i, "Yüklendi")
            end
        end
    ...
    for i=0, #plateTypes-1 do
        if i ~= 4 or (i == 4 and GetVehicleClass(plyVeh) == 18) or Config.allowGovPlateIndex then
            ...
            if tempPlateIndex == i then
                updateItem2Text("PlateIndexMenu", i, "Yüklendi")
            end
        end
    end
    ...
    --#[Vehicle Extras Menu]#--
    createMenu("VehicleExtrasMenu", "Vehicle Extras Customisation", "Extra Seç")
    for i=1, 12 do
    ...
    --#[Neons Menu]#--
    createMenu("NeonsMenu", "Neon Customisation", "Kategori Seç")
    ...
    for k, v in ipairs(vehicleNeonOptions.neonTypes) do
        ...
        updateItem2Text(v.name:gsub("%s+", "") .. "Menu", currentNeonState, "Yüklendi")
    ...
    for k, v in ipairs(vehicleNeonOptions.neonColours) do
        ...
        if currentNeonR == vehicleNeonOptions.neonColours[k].r and currentNeonG == vehicleNeonOptions.neonColours[k].g and currentNeonB == vehicleNeonOptions.neonColours[k].b then
            updateItem2Text("NeonColoursMenu", k, "Yüklendi")
        end
    end
    ...
    --#[Xenons Menu]#--
    createMenu("XenonsMenu", "Xenon Customisation", "Kategori Seç")
    ...
    updateItem2Text("HeadlightsMenu", currentXenonState, "Yüklendi")
    ...
    for k, v in ipairs(vehicleXenonOptions.xenonColours) do
        ...
        if currentXenonColour == v.id then
            updateItem2Text("XenonColoursMenu", v.id, "Yüklendi")
        end
    end
...
function MenuManager(state, repairOnly)
    if state then
        if currentMenuItem2 ~= "Yüklendi" then
            if isMenuActive("modMenu") then
                if currentCategory == 18 then --Turbo
                    if AttemptPurchase("turbo", currentMenuItemID) then
                        ...
                        updateItem2Text(currentMenu, currentMenuItemID, "Yüklendi")
            ...
            elseif isMenuActive("NeonsSideMenu") then
                ...
                elseif currentMenu == "WheelsMenu" then
                    ...
                    if currentWheelType == currentWheelCategory then
                        updateItem2Text(currentMenu, currentWheel, "Yüklendi")
                    end
