...
AddEventHandler('tgiann-mekanik:admin', function()
        local plyPed = PlayerPedId()
...
function SetupInteraction()
    ...
    if Config.UseRadial then
        if not radialMenuItemId then
            radialMenuItemId = exports['qb-radialmenu']:AddOption(6, {
                id = 'customs',
                title = 'Mekanik',
                icon = 'wrench',
                type = 'client',
...
RegisterNetEvent('mechanic:client:BillPlayer', function()
    ...
    if player ~= -1 and distance < 2.5 then
        ...
        if tonumber(dialog['bill']) > 0 then
            TriggerServerEvent("um-billing:server:bill", playerId, tonumber(dialog['bill']))
        else
            QBCore.Functions.Notify("Fiyat 0'dan yüksek olmalı", "error")
        end
    else
