...
if not IsDuplicityVersion() then --client
    ...
    sh.getPlayersKeyPreference = function(command, cb)
        local key = GetControlInstructionalButton(0, joaat('+sh_' .. command) | 0x80000000, true):sub(3)
    ...
    sh.sendLog = function(webhookURL, color, str, imgUrl)
        if webhookURL and webhookURL ~= '' then
            ...
            local data = {
                ...
                ['embeds'] = { {
                    ['title'] = 'store.sh-scripts.com',
                    ['url'] = 'https://store.sh-scripts.com/',
                    ['color'] = logColors[color] and logColors[color] or logColors['default'],
                    ['timestamp'] = os.date('!%Y-%m-%dT%H:%M:%SZ'),
                } },
                ['footer'] = {
                    ['text'] = 'store.sh-scripts.com',
