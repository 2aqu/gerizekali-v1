	...
	for k, v in pairs(Config.Locations) do
		if k == "vendingmachine" and Config.VendOverride then
			...
			for l, b in pairs(v["coords"]) do
				...
				local options = { { event = "jim-shops:ShopMenu", icon = (v["targetIcon"] or "fas fa-cash-register"), label = (v["targetLabel"] or "Market"), item = v["requiredItem"], job = v["job"] or nil, gang = v["gang"] or nil,
...
RegisterNetEvent('jim-shops:ShopMenu', function(data, custom)
	...
	ShopMenu[#ShopMenu + 1] = { icon = "fas fa-circle-xmark", header = "", txt = "Kapat", params = { event = "jim-shops:CloseMenu" } }
	...
	for i = 1, #products do
		...
		if not QBCore.Shared.Items[products[i].name:lower()] then
			...
			if products[i].price == 0 then price = "Free" else price = "Fiyat: $"..products[i].price end
			...
			local text = price.."<br>Ağırlık: "..(QBCore.Shared.Items[products[i].name].weight / 1000)..Config.Measurement
			if Config.Limit and not custom then
				if amount ~= 0 then	text = price.."<br>Stok: x"..amount.."<br>Ağırlık: "..(QBCore.Shared.Items[products[i].name].weight / 1000)..Config.Measurement
				else text = price.."<br>Stok Yok<br>Ağırlık: "..(QBCore.Shared.Items[products[i].name].weight / 1000)..Config.Measurement end
			end
...
RegisterNetEvent('jim-shops:Charge', function(data)
	...
	if QBCore.Shared.Items[data.item].weight == 0 then weight = "" else weight = "Ağırlık: "..(QBCore.Shared.Items[data.item].weight / 1000)..Config.Measurement end
	local settext = "- Sepet -<br><br>"
	if Config.Limit and data.amount ~= nil then settext = settext.."Adet: "..data.amount.."<br>" end
	settext = settext..weight.."<br> Adet Başı Fiyat: "..price.."<br><br>- Ödeme Şekli -"
	...
	local newinputs = {
		{ type = 'radio', name = 'billtype', text = settext, options = { { value = "cash", text = "Nakit" }, { value = "bank", text = "Banka" } } },
		{ type = 'number', isRequired = true, name = 'amount', text = 'Satın Al' } }

	local dialog = exports['qb-input']:ShowInput({ header = header, submitText = "Öde", inputs = newinputs })
	if dialog then
		if not dialog.amount then return end
		if Config.Limit and data.custom == nil then	if tonumber(dialog.amount) > tonumber(data.amount) then TriggerEvent("QBCore:Notify", "Bu üründe stok kalmadı", "error") TriggerEvent("jim-shops:Charge", data) return end end
