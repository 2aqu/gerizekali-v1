...
RegisterNetEvent('pedver:amk')
AddEventHandler('pedver:amk', function(ped, key)
        local skin = ped
...
function OpenShop(config, isPedMenu, shopType)
    lib.callback("illenium-appearance:server:hasMoney", false, function(hasMoney, money)
        if not hasMoney and not isPedMenu then
            lib.notify({
                title = "Başarısız",
                description = "Yeterli para yok. Gerekli Para: $" .. money,
                type = "error",
                position = Config.NotifyOptions.position
            })
            return
...
RegisterNetEvent("illenium-appearance:client:openClothingShopMenu", function(isPedMenu, key)
...
RegisterNetEvent("illenium-appearance:client:reloadSkin", function(bypassChecks)
    if IsPedBeingStunned(PlayerPedId(), 0) or IsPedSwimmingUnderWater(PlayerPedId()) or not bypassChecks and InCooldown() or Framework.CheckPlayerMeta() or cache.vehicle or IsPedFalling(cache.ped) then
        lib.notify({
            title = _L("commands.reloadskin.failure.title"),
            description = "Şuanda yenile kullanamazsın",
            type = "error",
            position = Config.NotifyOptions.position
        })
        return
