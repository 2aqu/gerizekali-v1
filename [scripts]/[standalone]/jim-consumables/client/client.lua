...
RegisterNetEvent('jim-consumables:Consume', function(itemName)
	...
	if type == "drink" or type == "alcohol" then string = "İçiyorsun " end
	if type == "food" then string = "Yiyorsun " end
	if type == "drug" then string = "Kullanıyorsun " end
	if consuming then
		cancelled = true
    ...
    if Config.UseProgbar then
        QBCore.Functions.Progressbar('jimmy_consume_', QBCore.Shared.Items[itemName].label.." "..string.."...", time, false, false, {disableMovement = false, disableCarMovement = false, disableMouse = false, disableCombat = true,}, {
            animDict = animDict,
            anim = anim,
            flags = 49,
        }, {}, {}, function() consuming = false end, function() end, itemName)
    else
