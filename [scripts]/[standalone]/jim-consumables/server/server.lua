...
QBCore.Functions.CreateUseableItem("airpods", function(source, item)
	local src = source
	if airpods then
		SetPedPropIndex(src, 2, 8, 0)
		airpods = false
		TriggerClientEvent('QBCore:Notify', src, 'Airpods\'lar çıkartıldı.')
	else
		SetPedPropIndex(src, 2, 1, 0)
		airpods = true
		TriggerClientEvent('QBCore:Notify', src, 'Airpods\'lar takıldı.', 'success')
	end
end)