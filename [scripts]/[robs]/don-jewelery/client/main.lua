        ...
        if cops >= Config.RequiredCops then
          if isStoreHit(k) or isStoreHacked() then
            ...
            QBCore.Functions.Progressbar("smash_vitrine", Lang:t('info.progressbar'), Config.WhitelistedWeapons[pedWeapon]["timeOut"], false, true, {
              ...
              TriggerServerEvent('ak4y-blackmarket:taskCountAdd', 4, 1)
              TriggerServerEvent('don-jewellery:server:SetTimeout', k)
                if not secondAlarm and not isStoreHacked() then 
                  if not Config.PSDispatch then
                    TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Soygun Sürüyor", false)
                  else
            ...
            QBCore.Functions.Notify('Mağaza Güvenliğini devre dışı bırakmadan devam edemezsin!', 'error')
          end
        else
          ...
          if QBCore.Functions.HasItem("thermite") then
            ...
            exports['ps-ui']:Thermite(function(success) -- success
              if success then
                ...
                QBCore.Functions.Notify("Thermite Yerleştirildi...", 'success', 4500)
                ...
                QBCore.Functions.Notify("Başaramadın, tekrar dene!", 'error', 4500)
                TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Kuyumcuda Şüpheli Aktivite", false)
              end
        ...
        for k, v in pairs(Config.Hacks) do
          ...
          if Dist <= 1.5 then
            if QBCore.Functions.HasItem("phone") then
              startHack()
              QBCore.Functions.Notify("Sisteme giriş yapılıyor...", 'success', 2500)
              -- if math.random(1, 100) <= 80 and not isWearingHandshoes() then
              ...
              exports['ps-ui']:VarHack(function(success)
                if success then
                  ...
                  TriggerServerEvent('don-jewellery:server:givekeycard')
                else
                  QBCore.Functions.Notify("Başaramadın, tekrar denemelisin!", 'error', 3500)
                  TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Kuyumcuda Şüpheli Aktivite", false)
                  stopHack()
  ...
  if isStoreHit(k) or isStoreHacked() then
    if isStoreHit(k)  and not isStoreHacked() then
      if not Config.OneStore then
        ...
        QBCore.Functions.Notify("Sigortalar Patladı! Kapılar ".. cooldownTime .. " dakikalığına açıldı.", 'success')
        TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Kuyumcu Yakınlarında Patlama Oldu...", false)
        unlockDoors(k)
        Wait(Config.Cooldown - warningTimer)
        QBCore.Functions.Notify("Acelete et! kapılar ".. warningTime .. "dakika içinde kitlenecek.", 'error')
        Wait(warningTimer)
      end
      if not checkRobberyTime() then
        lockDoors(k)
      ...
      if not Config.OneStore then 
        ...
        QBCore.Functions.Notify("Hack başarılı: Güvenlik sistemi devredışı..", 'success')
        TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Kuyumcu Güvenlik Sistemi Devre Dışı...", false)
      end
    end
    firstAlarm = false
    secondAlarm = false
  ...
  exports['qb-target']:AddBoxZone("jewelpc" .. k, v.coords, 0.4, 0.6, {
    ...
    maxZ= v.maxZ
      ...
      options = {
        ...
        type = "client",
        icon = 'fas fa-bug',
        label = 'Güvenlik Sistemini Hackle',
        item = 'phone',
        action = function()
            securityHack()
          end
