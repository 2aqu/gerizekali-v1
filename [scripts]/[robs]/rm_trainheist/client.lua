    ...
    while trainLoop do
        ...
        for k, v in pairs(Config['TrainHeist']['setupTrain']['containers']) do
            ...
            if dist <= 1.5 and not v['lock']['taken'] then
              -- TriggerEvent('cd_drawtextui:ShowUI', 'show', '[E] Kesmeye Başla')
              -- Citizen.Wait(1000)
            ...
            for x, y in pairs(v['golds']) do
                ...
                if dist <= 1.1 and not y['taken'] and not grabNow then
                  --  TriggerEvent('cd_drawtextui:ShowUI', 'show', '[E] Toplamaya Başla')
                  --exports['qb-core']:DrawText("[E] Toplamaya Başla")
...
function FinishHeist()
    ...
    QBCore.Functions.Notify('Çalıntı malı alıcıya teslim et. GPS’i kontrol et.', 'error')
...
function OpenContainer(index)
    QBCore.Functions.TriggerCallback('trainheist:server:hasItem', function(hasItem, itemLabel)
        if hasItem then
            ...
            QBCore.Functions.Notify('Gerekli item yok', 'error')
