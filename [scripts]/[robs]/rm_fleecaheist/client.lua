...
AddEventHandler('fleecaheist:client:policeAlert', function()
    TriggerEvent("Tgiann-PolisBildirim:BildirimGonder", "Küçük Banka Soygunu", false)
   -- mapblip(suc, x, y)
end)
...
AddEventHandler('fleecaheist:client:nearBank', function()
    ...
    if index ~= nil then
        ...
        QBCore.Functions.Notify('Yakınlarda banka yok!', 'error')
    end
end)
...
function Grab(index)
    QBCore.Functions.TriggerCallback('fleecaheist:server:hasItem', function()
        ...
        --else
           ...
           -- QBCore.Functions.Notify('Üzerinde çanta ve matkap olması lazım!', 'error')
       -- end
    end)
end
