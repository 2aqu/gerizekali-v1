function Modules.UI.DisplayDrift()
    if ConfigShared.PositionsCoords[ConfigShared.Position] ~= nil then
        ...
        Modules.UI.DrawTexts(baseX + 0.109375, baseY - 0.00259260892868, tostring(Modules.Utils.Comma_value(Modules.DriftCounter.CurrentPoints)) .." ~c~PUAN", true, 0.8, {250, 224, 64, alphaToUse}, Modules.UI.font["forza"], false, false)
        ...
        if ConfigShared.DisplayAngle then
            ...
            Modules.UI.DrawTexts(baseX + x, baseY + baseYToAdd - 0.0122, tostring(math.floor(Modules.DriftCounter.CurrentAngle) * 2) .."°", true, 0.4, {250, 224, 64, alphaToUse}, Modules.UI.font["forza"], false, false)
        end
    else
...
Citizen.CreateThread(function()
    if ConfigShared.UseDefaultUI then
        while true  do
            Wait(0)
            if not Modules.DriftCounter.IsEnabled then return end
