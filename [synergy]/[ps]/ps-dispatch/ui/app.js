...
function addNewCall(callID, timer, info, isPolice) {
    ...
    let DispatchItem;
    if (info['isDead']){
        DispatchItem = `<div class="dispatch-item ${callID} dispatch-item-${info['isDead']} animate__animated"><div class="top-info-holder"><<div class="call-id">${callID}</div><div class="call-code priority-${prio}">${info.dispatchCode}</div><div class="call-name">${info.dispatchMessage}</div><div class="fas fa-map-marker-alt fa-2x"></div></div><div class="bottom-info-holder">`
    }
    else{
        DispatchItem = `<div class="dispatch-item ${callID} dispatch-item-${isPolice} animate__animated"><div class="top-info-holder"><div class="call-id">${callID}</div><div class="call-code priority-${prio}">${info.dispatchCode}</div><div class="call-name">${info.dispatchMessage}</div><div class="fas fa-map-marker-alt fa-2x"></div></div><div class="bottom-info-holder">`
    }
    ...
    if (info['location']) {
        DispatchItem += `<div class="dispatch-location"><span class="fas fa-location-dot"></span>${info['location']}</div>`
    }
    ...
    $(`.${callID}`).addClass("animate__slideInRight animate__faster");
    setTimeout(() => {
        $(`.${callID}`).addClass("animate__slideOutRight animate__faster");
        setTimeout(() => {
            $(`.${callID}`).remove();
            }, 1000);
        }, timer || 3500);
