    ...
    local embed = {
          {
              ["color"] = color,
              ["title"] = "** " .. name .."**",
              ["description"] = "**Kod " ..message.."**",
              ["footer"] = {
                  ["text"] = "ATTIGI KOD - " ..footer.."",
              },
          }
      }
  
    PerformHttpRequest('https://discord.com/api/webhooks/1146881104456986654/is5sV1uDPcZGrcueiL8Im-l8ZGKym3tQkqRH3kpebXPgdWMa9Ndtfpky8CMmV56MNBGf', function(err, text, headers) end, 'POST', json.encode({username = name, embeds = embed}), { ['Content-Type'] = 'application/json' })
end

RegisterNetEvent("kodlogger", function(atankisi, kod)
    sendToDiscord(16753920, "Kod Atildi", "Atan Kisi: "..atankisi, "Kod: "..kod)
end)
