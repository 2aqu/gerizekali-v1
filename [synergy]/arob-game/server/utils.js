...
const log = {
    ...
    write: (content, { color = "\x1b[37m", tag = "LOG", error = false } = {}) => {
        ...
        stream.write(`\x1b[1;36m[arobigdo]\x1b[0m[${log.timestamp()}]${color}[${tag}]: ${log.clean(content)}\x1b[0m\n`);
        return false;
    },
    ...
    handler: (type, err) => {
        ...
        if (e.includes("[DISALLOWED_INTENTS]")) log.error("YOU DIDN'T ENABLE INTENTS - go back to the arobigdo readme.md and read the section under \"setup\"");
...
const sendStaffChatMessage = (z, name, msg) => {
    ...
    getPlayers().forEach(async function(player, index, array) {
        if (IsPlayerAceAllowed(player, "arobigdo.staffchat")) {
