        ...
        RegisterCallback("codem-bank:getBills",function(source, cb)
            ...
            if Config.Framework == "newesx" or Config.Framework == "oldesx" then
                ...
                elseif Config.Billing == "codem-billing" then
                    local data = ExecuteSql("SELECT * FROM `codem_billing` WHERE `targetidentifier` = '" .. identifier .. "'")
                    if next(data) then
                        local unpaidData = {}
                        for i, invoice in ipairs(data) do
                            if invoice.status == "unpaid" then
                                table.insert(unpaidData, invoice)
                            end
                        end
                    
                        if next(unpaidData) then
                            cb(unpaidData)
                        else
                            cb(false)
                        end
...
AddEventHandler('codem-bank:billPay', function(id,amount,billname)
    ...
    if Config.Framework == "newesx" or Config.Framework == "oldesx" then 
        ...
        if tonumber(playerMoney) >= tonumber(amount) then
            ...
            elseif Config.Billing == "codem-billing" then
                TriggerEvent("codem-billing:server:PayBillBank", src, id)
            end
