...
const Hud = {
    ...
    methods: {
            ...
            switch (event.data.action) {
                ...
                case "MONEY_CHANGE":
                    if(this.moneyChange[event.data.payload.moneyType].timeout){
                        clearTimeout(this.moneyChange[event.data.payload.moneyType].timeout)
                    }
                    this.moneyChange[event.data.payload.moneyType].minus = event.data.payload.minus
                    this.moneyChange[event.data.payload.moneyType].amount = event.data.payload.amount
        ...
        getDate() {
            let date = new Date().toLocaleDateString(this.countryCode, { month: "short", day: "numeric", hour: "numeric", minute: "numeric" })
            return date
        },
    },
    components: {
    ...
    computed: {
        ...
        getMaxWidth() {
            ...
            let name = `${this.player_name} - ${this.hudSettings.streamerMode && !this.hudSettings.showMyPlayerId ? '**' : this.id}`            
            const jobs = [displayTextWidth(second_job, "5px Gilroy"), displayTextWidth(first_job, "5px Gilroy"), displayTextWidth(name, "8px Gilroy")]
            const widths = [...jobs, displayTextWidth(this.waterMarkInformations.servername, "6px Gilroy"), displayTextWidth(this.accounts.bank.toString(), "18px Gilroy"), displayTextWidth(this.accounts.cash.toString(), "18px Gilroy"), displayTextWidth(this.accounts.black_money.toString(), "15px Gilroy"), displayTextWidth(this.accounts.vip_money.toString(), "15px Gilroy"), displayTextWidth(this.accounts.boss_money.toString(), "15px Gilroy")]
        ...
        getNameBoxStyle() {
            if (this.hudSettings.nameBoxPosition.left && this.hudSettings.nameBoxPosition.top) {
                return {
        ...
        getBankBoxStyle() {
            if (this.hudSettings.bankBoxPosition.left && this.hudSettings.bankBoxPosition.top) {
                return {
        ...
        getServerInfoBoxStyle() {
            if (this.hudSettings.serverInfoBoxPosition.left && this.hudSettings.serverInfoBoxPosition.top) {
                return {
        ...
        getCashBoxStyle() {
            if (this.hudSettings.cashBoxPosition.left && this.hudSettings.cashBoxPosition.top) {
                return {
        ...
        getBossMoneyBoxStyle() {
            if (this.hudSettings.bossMoneyBoxPosition.left && this.hudSettings.bossMoneyBoxPosition.top) {
                return {
        ...
        getBlackMoneyBoxStyle() {
            if (this.hudSettings.blackMoneyBoxPosition.left && this.hudSettings.blackMoneyBoxPosition.top) {
                return {
        ...
        getVipMoneyBoxStyle() {
            if (this.hudSettings.vipMoneyBoxPosition.left && this.hudSettings.vipMoneyBoxPosition.top) {
                return {
