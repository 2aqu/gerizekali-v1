Dyno = function(data, index)
	local dynoprop = not config.useMlo and DoesEntityExist(ramp) and GetEntityModel(ramp) == config.dynoprop or
	config.useMlo
	...
	if candyno and not dynotaken then
		indyno = true
		exports["mHud"]:disable()
		lib.notify({
			title = 'Dynamometer Mode',
			type = 'success'
		})
		lib.showTextUI('[YUKARI OK] - Vites At  \n [AŞAĞI OK] - Vites Düşür  \n [G] Dynodan çık',
			{ icon = 'gamepad', position = "top-center" })
	...
	elseif not dynotaken and not dynodriver then
		lib.notify({
			title = 'Dynoyu başkası kullanıyor!',
			type = 'error'
		})
	end
end
...
SetVehicleManualGears = function(vehicle, dyno, auto, eco)
	...
	while fInitialDriveMaxFlatVel == nil or fDriveInertia == nil or nInitialDriveGears == nil or fInitialDriveForce == nil do
		Wait(1) end
	...
	SetVehicleHandlingInt(vehicle, "CCarHandlingData", "strAdvancedFlags", vehicleflags + 0x20000 + 0x200 + 0x1000 + 0x10 +
	0x40)
	manual = true
			...
			if afr > 14.0 and rpm > 0.5 then
				...
				if temp > 120.0 then
					SetVehicleEngineHealth(vehicle,
						not explode and enginelife < 100.0 and 0.0 or explode and 0.0 or enginelife > 1000.0 and 1000.0 or
						enginelife)
				end
			end
		end
	...
	Citizen.CreateThreadNow(function()
		if dyno then
			...
			ent:set('startdyno', { ts = GetGameTimer(), platform = zoffset, dyno = true, inertia = default_fDriveInertia },
				true)
		else
			gear_ratio = vehicle_gear_ratio[maxgear][1] * (1 / 0.9)
			ent:set('gearshift',
				{ gear = 1, gearmaxspeed = (((maxspeed * 1.32) / 3.6) / gear_ratio), flatspeed = (maxspeed / gear_ratio), driveforce = (driveforce * gear_ratio) },
				true)
		end
		...
		while invehicle and manual do
			...
			if throttle > 0.0 and not dyno and not switching and clutching < 0.1 then
				if gear > (auto and 0 or 0) and gear <= maxgear and rpm < 0.99999 or customgears and rpm < 0.99999 then
					...
					if currentmaxgear > 1 or true then
						...
						SetVehicleHandlingFloat(vehicle, "CHandlingData", "fInitialDriveForce",
							(driveforce * gear_ratio) * throttle)                                                    -- last multiplication for steering wheel pedals
						SetVehicleHandlingInt(vehicle, "CHandlingData", "nInitialDriveGears", 1)
						SetVehicleMaxSpeed(vehicle, gearmaxspeed + 1.0)
						ModifyVehicleTopSpeed(vehicle, 1.0)
						SetVehicleHandlingInt(vehicle, "CCarHandlingData", "strAdvancedFlags",
							vehicleflags + 0x400000 + 0x20000 + 0x4000000 + 0x20000000)
						SetVehicleHandlingFloat(vehicle, "CHandlingData", "fDriveInertia",
							default_fDriveInertia * throttle)
					end
				...
				elseif gear == 1 then
					SetVehicleHandlingInt(vehicle, "CCarHandlingData", "strAdvancedFlags",
						vehicleflags + 0x20000 + 0x200 + 0x1000)
				end
			elseif not dyno and GetControlNormal(0, 72) < 0.1 and gear_ratio and rpm > 0.3 and clutching < 0.1 then
				...
				switch = false
				SetVehicleHandlingFloat(vehicle, "CHandlingData", "fDriveInertia",
					0.1 + (default_fDriveInertia / gear) * (1.0 - rpm))
				SetVehicleHandlingFloat(vehicle, "CHandlingData", "fInitialDriveForce",
					(driveforce * (gear / maxgear)) * (1 - rpm))
			end
			...
			if clutch or clutching > 0.1 then
				...
				SetVehicleHandlingInt(vehicle, "CCarHandlingData", "strAdvancedFlags", vehicleflags + 0x20000 + 0x200 +
				0x1000)
		...
		if dyno then
			Entity(vehicle).state:set('startdyno',
				{ ts = GetGameTimer(), platform = zoffset, dyno = false, inertia = default_fDriveInertia }, true)
			dyno = false
		...
		if config.sandboxmode then
			ent:set('vehiclestatreset',
				{ strAdvancedFlags = GetVehicleHandlingInt(vehicle, 'CCarHandlingData', 'strAdvancedFlags'), fInitialDriveMaxFlatVel =
				fInitialDriveMaxFlatVel, fInitialDriveForce = fInitialDriveForce, fDriveInertia = tuning_inertia or
				default_fDriveInertia, nInitialDriveGears = nInitialDriveGears }, true)
		else
			ent:set('vehiclestatreset',
				{ strAdvancedFlags = vehicleflags, fInitialDriveMaxFlatVel = maxspeed, fInitialDriveForce = driveforce, fDriveInertia =
				inertia, nInitialDriveGears = maxgear }, true)
		end
	...
	Upshift = function()
		if manual and gear + 1 <= maxgear then
			...
			if dyno then
				...
				ent:set('gearshift',
					{ gear = gear, gearmaxspeed = gearmaxspeed, flatspeed = (maxspeed / nextgear_ratio), driveforce = (driveforce * nextgear_ratio) },
					true)
			end
			SetVehicleHandlingInt(vehicle, "CCarHandlingData", "strAdvancedFlags", vehicleflags + 0x20000 + 0x200 +
			0x1000)
	...
	DownShift = function()
		if manual and vehicle_gear_ratio[maxgear][gear - 1] then
			...
			ForceVehicleSingleGear(vehicle, nextgearspeed, dyno)
			ent:set('gearshift',
				{ gear = gear, gearmaxspeed = nextgearspeed, flatspeed = (maxspeed / nextgear_ratio), driveforce = (driveforce * nextgear_ratio) },
				true)
			if GetResourceState('renzu_turbo') == 'started' then
				exports.renzu_turbo:BoostPerGear(boostpergear[gear] or 1.0)
			end
