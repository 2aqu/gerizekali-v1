	...
	for k, enable in pairs(config.upgradevariation) do
		if enable then
			table.insert(options,
				{
					icon = 'box',
					label = k:gsub("^%l", string.upper) .. ' Package',
					description = 'Şu paketi tak: ' ..
						k,
					args = k
				})
		end
...
Options = function(data, shop, job)
	...
	if data.tires then
		for k, v in pairs(config.tires) do
			local desc = 'Bu lastiğe geç: ' .. v.label
			if config.purchasableUpgrade then
				desc = 'Fiyat: $ ' .. v.cost or 100000
			end
			table.insert(options,
				{
	...
	elseif data.drivetrain then
		for k, v in pairs(config.drivetrain) do
			local desc = 'Bu drivetraine geç: ' .. v.label
			if config.purchasableUpgrade then
				desc = 'Fiyat: $ ' .. v.cost or 100000
			end
			table.insert(options,
	...
	elseif data.extras then
		for k, v in pairs(config.extras) do
			local desc = 'Tak: ' .. v.label
			if config.purchasableUpgrade then
				desc = 'Fiyat: $ ' .. v.cost or 100000
			end
			table.insert(options,
				{ icon = imagepath .. '' .. v.item .. '.png', label = v.label, description = desc, args = v.item })
		end
	elseif data.turbo then
		...
		for k, v in pairs(turbos) do
			local desc = 'Tak: ' .. v.label
			if config.purchasableUpgrade then
				desc = v.cost and 'Fiyat: $ ' .. v.cost or 100000
			end
			table.insert(options,
				{
					icon = imagepath .. '' .. v.item .. '.png',
	...
	elseif data.customengine then
		for k, v in pairs(data.value) do
			local desc = 'Motoru Tak: ' .. v.label
			if config.purchasableUpgrade then
				desc = v.cost and 'Fiyat: $ ' .. v.cost or 100000
			end
			table.insert(options,
				{ icon = imagepath .. 'engine.png', label = v.label or 'engine', description = desc, colorScheme = 'blue', args = { engine = true, item = v.soundname } })
		end
	elseif job and not data.ecu and not data.mileage then
		if data.state == data.installed then
			local desc = 'Onar: ' .. data.label
			if config.purchasableUpgrade then
				desc = 25000
			end
			table.insert(options,
				{ icon = imagepath .. '' .. data.installed .. '.png', label = 'Repair ' .. data.label, description = desc, args = { name = 'repairparts', part = data.installed } })
		else
			local desc = 'Değiştir: ' .. data.label
			if config.purchasableUpgrade then
				desc = 25000
			end
			table.insert(options,
				{
					icon = imagepath .. '' .. data.state .. '.png',
					label = 'OEM Tak ',
					description = desc,
					args = data
						.state
				...
				table.insert(options,
					{
						...
						label = v.item == data.installed and
							'Onar: ' .. v.label or 'Şuna tak: ' .. v.label,
						description = desc,
						args = v.item == data
							.installed and { name = 'repairparts', part = data.installed, state = v.state } or v.item
					})
			end
		end
	elseif job and not data.ecu then
		options = {
			--{ icon = imagepath .. '' .. data.part .. '.png', label = 'Yağ Değiştir', description = 'Restore to 0 Mileage', args = data.part },
		}
		hasmenu = true
	end
	if job and data.ecu then
		...
		if ecu[plate] then
			table.insert(options,
				{
					icon = imagepath .. 'engine.png',
					label = 'Yeni Profil',
					description = 'Yeni tuning profili oluştur',
					colorScheme =
					'blue',
					args = { tune = true, profile = 'new' }
				})
					...
					table.insert(options,
						{
							icon = imagepath .. 'engine.png',
							label = name,
							description = 'Profil yükle: ' .. name,
							colorScheme =
							'blue',
							args = { tune = true, profile = name, data = tuning }
						})
				end
			end
		else
			table.insert(options,
				{
					icon = imagepath .. 'engine.png',
					label = ' Programlanabilir ECU',
					description = 'Programlanabilir ECU Tak',
					colorScheme =
					'blue',
					args = data.part
				})
	...
	if hasmenu then
		...
		lib.registerMenu({
			id = 'upgrade_options',
			title = config.purchasableUpgrade and config.jobmanagemoney and 'Dükkan parası: ' .. jobmoney or 'Parça Seçenekleri',
			position = 'top-left',
			...
			onCheck = function(selected, checked, args)
				...
				if config.freeupgrade or hasitem then
					...
					local required = config.purchasableUpgrade and 'money' or 'item'
					lib.notify({
						description = 'Bu eşyaya sahip değilsin: ' .. required,
						type = 'error'
					})
				end
			end,
			...
			if type(args) == 'table' and args.tune then
				if args.profile == 'new' then
					...
					local options = {
						{ type = "input",  label = "Profil Adı",     placeholder = "Drag Ayarı" },
						{ type = "slider", label = "Avans",  min = -0.5,                      max = 1.5, step = 0.001, default = 1.0 },
						{ type = "slider", label = "Map",       min = -0.5,                      max = 1.5, step = 0.001, default = 1.0 },
						{ type = "slider", label = "Vites Tepkimesi",    min = -0.5,                      max = 1.5, step = 0.01,  default = 1.0 },
						{ type = "slider", label = "Diferansiyel Oranı", min = -0.5,                      max = 1.5, step = 0.01,  default = 1.0 },
					}
					...
					if hasturbo then
						for i = 1, totalgears do
							table.insert(options,
								{ type = "slider", label = "Vites " .. i .. ' Boost', min = -0.5, max = 1.5, step = 0.01, default = 1.0 })
						end
					end
					for i = 1, totalgears do
						table.insert(options,
							{ type = "input", label = "Vites " .. i .. ' Oranı', default = config.gears[maxgear][i] })
					end
					local input = lib.inputDialog('Yeni Tuning Profili', options)
					local boostpergear = {}
					local gear_ratio = {}
					if hasturbo then
						for i = 6, #input do
					...
					if input and input[1] then
						lib.notify({
							description = input[1] .. ' Profili uygulandı.',
							type = 'success'
						})
						lib.callback.await('renzu_tuners:Tune', false,
							{ vehicle = NetworkGetNetworkIdFromEntity(vehicle), profile = input[1], tune = { acceleration = input[2], topspeed = input[5], engineresponse = input[3], gear_response = input[4], boostpergear = boostpergear, gear_ratio = gear_ratio } })
						Wait(200)
						HandleEngineDegration(vehicle, Entity(vehicle).state, plate)
						local plate = string.gsub(GetVehicleNumberPlateText(GetVehiclePedIsIn(cache.ped)), '^%s*(.-)%s*$',
							'%1'):upper()
						vehiclestats, vehicletires, mileages, ecu = lib.callback.await('renzu_tuners:vehiclestats', 0,
							plate)
					else
						lib.notify({
							description = 'Tune kaydedilmedi.',
							type = 'error'
						})
					end
				else
					local hasturbo = Entity(vehicle).state.turbo
					local options = {
						{ type = "slider", label = "Avans",  min = -0.5, max = 1.5, step = 0.001, default = args.data['acceleration'] },
						{ type = "slider", label = "Map",       min = -0.5, max = 1.5, step = 0.001, default = args.data['engineresponse'] },
						{ type = "slider", label = "Vites Tepkimesi",    min = -0.5, max = 1.5, step = 0.01,  default = args.data['gear_response'] },
						{ type = "slider", label = "Diferansiyel Oranı", min = -0.5, max = 1.5, step = 0.01,  default = args.data['topspeed'] },
					}
					local totalgears = GetVehicleHighGear(vehicle)
					local maxgear = GetVehicleHighGear(vehicle)
					if hasturbo then
							...
							table.insert(options,
								{
									...
									step = 0.01,
									default =
										args.data['boostpergear'][i] or 1.0
								})
						end
					end
					for i = 1, totalgears do
						table.insert(options,
							{
								type = "input",
								label = "Vites " .. i .. ' Oran',
								default = args.data['gear_ratio'][i] or
									config.gears[maxgear][i]
							})
					end
					local input = lib.inputDialog('Profil ' .. args.profile, options)
					local boostpergear = {}
					local gear_ratio = {}
					if hasturbo then
						for i = 5, #input do
					...
					if input and input[1] then
						...
						lib.callback.await('renzu_tuners:Tune', false,
							{ vehicle = NetworkGetNetworkIdFromEntity(vehicle), profile = args.profile, tune = { acceleration = input[1], topspeed = input[4], engineresponse = input[2], gear_response = input[3], boostpergear = boostpergear, gear_ratio = gear_ratio } })
						Wait(200)
						HandleEngineDegration(vehicle, Entity(vehicle).state, plate)
						local plate = string.gsub(GetVehicleNumberPlateText(GetVehiclePedIsIn(cache.ped)), '^%s*(.-)%s*$',
							'%1'):upper()
				...
				if name == 'repairparts' then
					...
					local success = lib.callback.await('renzu_tuners:RepairPart', false, percent,
						ismetadataSupport == false)
					if success == 'item' then
						return lib.notify({
							description =
							'Failed to repair.  \n  You dont have a repair item',
							type = 'error'
						})
					end
					if success then
						...
						lib.notify({
							description = 'Repair Success.  \n  Repair kit Durability is ' .. durability,
							type =
							'success'
						})
						...
						return lib.notify({
							description =
							'Failed to repair.  \n  the current repair parts cannot repair this percentage',
							type = 'error'
						})
					end
				else
...
CheckVehicle = function(menu, shop)
	...
	table.insert(options,
		{
			icon = 'road',
			label = 'Kilometre Sayacı',
			description = ' Aracın şu anki kilometre durumu',
			progress = 100 -
				(ent.mileage / 10000) * 100,
			colorScheme = 'blue',
			args = { part = ent['racing_oil'] and 'racing_oil' or 'engine_oil', upgrade = 'racing_oil', mileage = Entity(vehicle).state.mileage, label = 'Mileage' }
		})
	...
	if menu then
			...
			table.insert(options,
				{
					...
					label = 'Forced Induction (' .. turbo .. ')',
					description =
					' Takılmış Özel Türbin',
					progress = ent['turbo'] and ent['turbo'].durability or 100,
					colorScheme =
					'blue',
					args = { turbo = true, label = 'Forced Induction', value = { 'turbostreet', 'turbosports', 'turboracing', 'turboultimate' } }
				})
		end
			...
			table.insert(options,
				{
					...
					label = 'Nitros Oxide Sistemi',
					description = ' Takılı Nitrolar',
					colorScheme =
					'black',
					args = { nitro = true, label = 'Nitro', value = { 'nitro50shot', 'nitro100shot', 'nitro200shot' } }
				})
		end
		table.insert(options,
			{
				...
				description =
				'Şimdiki lastik sağlığı',
				progress = ent?.tires?.tirehealth[1] or 100,
				colorScheme = 'blue',
				args = { tires = true, label = 'Tires', type = tiretype }
			})
		table.insert(options,
			{
				icon = imagepath .. 'oem_gearbox.png',
				label = 'Çekiş Türü: ' .. drivetraintype,
				description =
				'Lastik tipi değiştir',
				colorScheme = 'black',
				args = { drivetrain = true, label = 'Çekiş Türü', type = drivetraintype }
			})
		table.insert(options,
			{
				icon = imagepath .. 'kers.png',
				label = 'Gelişmiş',
				description = ' Gelişmiş Modifikasyonlar',
				colorScheme =
				'black',
				args = { extras = true, label = 'Gelişmiş Modifikasyonlar', value = ent.extras }
			})
		if GetResourceState('renzu_engine') == 'started' then
			...
			table.insert(options,
				{
					icon = imagepath .. 'engine.png',
					label = 'Motor (OEM) (şu anki: ' .. engine .. ')',
					description =
					' Swaplan Engines',
					colorScheme = 'black',
			...
			table.insert(options,
				{
					icon = imagepath .. 'engine.png',
					label = 'Motor (Custom) (şu anki: ' .. engine .. ')',
					description =
					'Swaplanmış Motorlar',
					colorScheme = 'black',
	...
	table.insert(options,
		{ icon = imagepath .. 'engine.png', label = 'ECU', description = 'Yazılım Preset', colorScheme = 'black', args = { part = 'ecu', ecu = true, label = 'ECU', value = true } })

	lib.registerMenu({
		id = 'checkvehicle',
		title = menu and '🛠️ Araç Yükseltmeleri' or 'Motor Durumu',
		position = 'top-left',
		imageSize = 'large',
		onClose = function(keyPressed)
			...
			table.insert(options,
				{
					icon = '',
					label = v.affects,
					description = 'Şu anki performans: ' .. v.affects,
					progress = prog or
						100,
					colorScheme = 'blue'
				})
...
TuningMenu = function()
	...
	if not activeprofile and not config.sandboxmode then
		lib.notify({
			description = 'Araçta ECU Mevcut Değil!',
			type = 'error'
		})
		return
	end
	...
	for k, v in ipairs(config.tuningmenu) do
		table.insert(options,
			{ icon = v.icon, label = v.label, description = v.description, args = { handling = v.handlingname, label = v.label, type = v.type, min = v.min, max = v.max, attributes = v.attributes } })
	end
	local totalgears = GetVehicleHighGear(vehicle)
	local maxgear = GetVehicleHighGear(vehicle)
	lib.registerMenu({
					...
					table.insert(options,
						{
							...
							step = v.step,
							default =
								activeprofile[v.name] or v.default
						})
				end
			elseif args.type == 'turbo' then
				for i = 1, totalgears do
					table.insert(options,
						{
							type = "slider",
							label = i .. '. Vites Boostu',
							min = -0.5,
							max = 1.5,
							step = 0.01,
							default =
								activeprofile.boostpergear[i] or 1.0
						})
				end
			elseif args.type == 'gearratio' then
				for i = 1, totalgears do
					table.insert(options,
						{ type = "input", label = "Vites " .. i .. ' Oranı', default = activeprofile.gear_ratio[i] })
				end
					...
					table.insert(options,
						{
							...
							step = v.step,
							default =
								activeprofile.suspension[v.label] or 1.0
						})
				end
			end
		else
				...
				table.insert(options,
					{
						...
						default = HandlingGetter(v.type,
							args.handling, v.label)
					})
			end
		end
		local profile = activeprofile?.profile or 'SANDBOX MODE'
		local input = lib.inputDialog('Profil Adı: ' .. profile, options)
		...
		if not config.sandboxmode then
			lib.callback.await('renzu_tuners:Tune', false,
				{ vehicle = NetworkGetNetworkIdFromEntity(vehicle), profile = activeprofile.profile, tune = activeprofile })
		else
			for k, v in ipairs(args.attributes) do
		...
		table.insert(options,
			{
				icon = '',
				label = v.label,
				description = v.label .. ' durumu',
				progress = v.health or 100,
				colorScheme =
				'blue'
			})
	end
	local wheeltype = ent.tires?.type or 'Varsayılan OEM'
	lib.registerMenu({
		id = 'wheelstatus',
		title = 'Lastik Durumu : (' .. wheeltype .. ')',
		position = 'bottom-right',
		options = options
	}, function(selected, scrollIndex, args)
...
ContextMenuOptions = function(stash, entity, vehicle)
	...
	if #(GetEntityCoords(vehicle) - GetEntityCoords(cache.ped)) > 3 then
		lib.notify({ type = 'error', description = 'Bir araca yakın değilsin!' })
		return
	end
	...
	for k, v in pairs(items) do
		local name = v.metadata?.label or 'Motor'
		...
		table.insert(options, {
			...
			onSelect = function()
				...
				enginemodel = CreateObject(`prop_car_engine_01`, stand.x + 0.27, stand.y - 0.2, stand.z + z, true, true,
					true)
				while not DoesEntityExist(enginemodel) do Wait(1) end
				SetEntityCompletelyDisableCollision(enginemodel, true, false)
				AttachEntityToEntity(enginemodel, entity, 0, 0.0, -1.25, z, 0.0, 90.0, 0.0, true, false, false, false, 70,
					true)
				while z > 0.2 and DoesEntityExist(enginemodel) do
					Wait(1)
					z -= 0.003
					AttachEntityToEntity(enginemodel, entity, 0, 0.0, -1.25, z, 0.0, 90.0, 0.0, true, false, false, false,
						70, true)
				end
				lib.progressBar({
					duration = 8000,
					label = 'Motoru Takıyorsun...',
					useWhileDead = false,
					canCancel = true,
					disable = {
						car = true,
				...
				lib.callback.await('renzu_tuners:RemoveEngineStorage', false,
					{ stash = stash, name = v.name, slot = v.slot, metadata = v.metadata })
...
CraftOption = function(items, craft, label)
	...
	for k2, item in pairs(items) do
		...
		table.insert(materials, 'Şans: ' .. chance .. '%')
		table.insert(options, {
			title = item.label,
			metadata = materials,
			...
			onSelect = function()
				...
				if hasitems then
					...
					local success = lib.callback.await('renzu_tuners:Craft', false, slots, requiredata, item,
						craft == 'engine' and item)
					if success then
