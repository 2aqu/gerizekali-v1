...
local function AttemptRappel()
    ...
    if not DoesVehicleAllowRappel(helicopter.entity) then
        QBCore.Functions.Notify(Config.Localisation.Notification.CannotRappelFromHeli , "error")
    ...
    if not isInCorrectSeat then
        QBCore.Functions.Notify(Config.Localisation.Notification.CannotRappelFromSeat , "error")
    ...
    if not foundHeight or coords.z - groundZ > 35.0 then
        QBCore.Functions.Notify(Config.Localisation.Notification.ToHighToRappel , "error")
    ...
    if lastRappelKeyPress == 0 or (lastRappelKeyPress + 1000 < GetGameTimer()) then
        ...
        QBCore.Functions.Notify(Config.Localisation.Notification.ConfirmRappel)
        ...
        QBCore.Functions.Notify(Config.Localisation.Notification.Rappeling , "success")
...
local function ToggleSpotlight()
    ...
    if not CanHelicopterUseSpotlight(helicopter.model) then
        QBCore.Functions.Notify(Config.Localisation.Notification.NoSpotlight , "error")
    ...
    if spotlight.active then
        ...
        if GlobalState.heliSpotlightsActive >= Config.MaxAmountOfSpotlights then
            ...
            QBCore.Functions.Notify(Config.Localisation.Notification.SpotlightGlobalLimit , "error")
...
AddEventHandler('helicam:enterCamera', function(state)
    if state then
        ...
        QBCore.Functions.Notify(Config.Localisation.Notification.CameraInUse , "error")
...
RegisterCommand('helicam', function()
    ...
    if not inHeliCam then
        ...
        if IsPedInAnyHeli(playerPed) or IsPedInAnyPlane(playerPed) then
            ...
            elseif message then
                QBCore.Functions.Notify(Config.Localisation.Notification[message])
