...
function FinishRace()
    ...
    TriggerServerEvent('cw-racingapp:server:FinishPlayer', CurrentRaceData, CurrentRaceData.TotalTime,
        CurrentRaceData.TotalLaps, CurrentRaceData.BestLap, class, GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(PlayerPedId()))))
...
RegisterNetEvent("cw-racingapp:Client:RaceResultMenu", function(data)
    ...
    if #raceResults > 0 then
        ...
        for i, result in ipairs(raceResults) do
            ...
            local vehicleaq = GetDisplayNameFromVehicleModel(GetEntityModel(GetVehiclePedIsIn(PlayerPedId())))
            menu[#menu + 1] = {
                header = header .. result.RacerName,
                text = 'Time: '.. MilliToTime(result.TotalTime)..' | BL: '..MilliToTime(result.BestLap).. ' | Vehicle: '..vehicleaq .. ' | Class: '..result.CarClass,
                disabled = true
            }
        end
        exports['qb-menu']:openMenu(menu)
...
exports('yarisaq', function()
    return CurrentRaceData.Started
end)
