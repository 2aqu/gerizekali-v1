    ...
    $.post('https://cw-racingapp/UiFetchCurrentRace', function(result){
        if (result) {
            if (result.raceId) {
                ...
                let body = `<div></br> Toplam Tur: ${laps} </div>`
                let footer = `<div>Katılan Yarışçılar: ${result.racers}</div>`
        ...
        $.each(FilterResultsByClass(Tracks[selectedTrack].Records, selectedClass), function(i, val) {
            ...
            let element = `
                <tr id="${i}" value="${i}" >
                    ...
                    <td class="result-row">
                        <span class="result-holder">
                            <span class="result-header"> Süre </span>
                            <span class="result-value"> ${ secondsTimeSpanToHMS(val.Time)} </span>
                        </span>
                    </td>
                    <td class="result-row">
                        <span class="result-holder">
                            <span class="result-header"> Araç </span>
...
function PopulateAvailableTracks() {
    ...
    $.each(AvailableTracks, function(i, track) {
        if (!curatedOnly || (curatedOnly && track.Curated)) {
            let setupTrackButton = `<button class="action-button small" onclick="handleSelectRaceSetup('${track.RaceId}', '${track.RaceName}')" id="setup-start-button"><span id="create-tab">Oluştur</span></button>`
            let showTrack = `<button class="action-button secondary-button small" onclick="handleShowTrack('${track.RaceId}')" id="setup-start-button"><span id="create-tab">Pisti Göster</span></button>`
    ...
    $.post('https://cw-racingapp/UiGetMyTracks', function(result){
        if (result) {
            ...
            $.each(result, function(i, track) {
                ...
                let editTrackButton = `<button class="action-button secondary-button small" onclick="EditTrack('${track.RaceId}', '${track.RaceName}')" id="mytracks-edit-button"><span id="create-tab">Düzenle</span></button>`
                ...
                let deleteTrackButton = `<button class="action-button secondary-button small" onclick="DeleteTrack('${track.RaceId}', '${track.RaceName}')" id="mytracks-delete-button"><span id="create-tab">Sil</span></button>`
                let element = `
                    <div id="${track.RaceId}-${i}" class="big-card">
                        <div class="card-content">
                            ...
                            <div class="card-footer standardGap inline">${!track.Curated ? editTrackButton:''}${deleteTrackButton}</div>
                        </div>
                    </div>
                `;
