...
CreateThread(function()
	if Config.MakeStores then
		for k, v in pairs(Config.Stores) do
			...
			if v.info.showBlip then Blips[#Blips+1] = makeBlip({coords = v.info.coords, sprite = 478, col = 81, name = "Sandalye Dünyası"}) end
			Targets["ChairStore"..k] =
				...
				{ options = { { event = "jim-chairs:openShop", icon = "fas fa-chair", label = "Mağazaya Gözat", store = v, ped = Peds[#Peds] }, },
					distance = 2.0
				})
		end
	end
...
RegisterNetEvent("jim-chairs:Use", function(item)
	if IsPedInAnyVehicle(PlayerPedId(), false) then triggerNotify(nil, "Araçtayken bunu kullanamazsın!", "error") return end
