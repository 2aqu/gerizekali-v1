...
window.addEventListener('message', function(event) {
    ...
    if (data.action === "OpenBoxingMenu"){
      ...
      if (Areatable.bet == true){
        for (let i=1; i<3; i++) {
          ...
          if (document.getElementById("player_bet"+i) == undefined){
            ...
            <div class="input-group input-group-lg mt-3 bet_con" style="width: 200px;">
                <input class="form-control" placeholder="MİKTAR" id="in_bet${i}"></input>
            </div>
            <button class="basic_btn px-3 mt-3" id="player_bet${i}" onclick="Bet(id)">BAHİS</button>
            `)
          }
        }
        ...
        function CountbackBet(){
          BetTimerRun = true
          $(`.bet_countback`).html(Areatable.bettimer)
          setTimeout(function(){
    ...
    if (data.action === "Countback"){
      ...
      if (WinnerNum == false){
        ...
        $(".end_notify .winner").html("KAZANAN")
...
function RefreshBoxingMenu(){
  ...
  if (Areatable.started == false || Areatable.bet == true){
    for (let i=1; i<3; i++) {
      ...
      $("#player_container"+i).append(`
      <button class="basic_btn px-3 mt-3" id="player_btn${i}" onclick="Join(id)">KATIL</button>
      `)
    }
...
function ShowPlayerDetails(k){
  ...
  if (Areatable[k].id == PlayerId){
    ...
    <button class="back_btn px-3 mt-3" id="player_btn${number}" onclick="Leave(id)">AYRIL</button>
    `)
  }
  else{
    $("#player_container"+number).append(`
    <button class="basic_btn px-3 mt-3" id="player_btn${number}" disabled onclick="Join(id)">KATIL</button>
    `)
  }
}
...
function StartButtonCheck(){
  if(Areatable.player1.id != null && Areatable.player2.id != null && PlayerInGame){
    ...
    document.getElementById("start_btn").innerHTML = "BAŞLAT"
  }
