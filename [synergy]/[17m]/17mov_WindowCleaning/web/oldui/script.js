...
window.addEventListener('message', function (event) {
    ...
    if (action == "OpenWorkMenu") {
        ...
        $(".tutorial").fadeIn(250)
    } else if (action == "show2Tutorial") {
        $(".secondTutorial").fadeIn(250)
    } else if (action == "Init") {
...
function Close2Tutorial() {
    $(".secondTutorial").fadeOut(250)
    FocusOff()
}
...
function startRandomCleaning() {
    cleaningActive = true
    $(".cleaning").css("cursor", "url('oldui/imgs/tool.png') 64 10, auto")
    $(".window").html('<img id="dirt" src="' + "oldui/imgs/surface" + (Math.floor(Math.random() * 8) + 1) + ".png" + '" height="100%" width="100%">')
