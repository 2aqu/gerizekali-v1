...
window.addEventListener('message', function (event) {
    ...
    if (action == "addNewMember") {
        ...
        $("#counter").fadeOut(250)
        $(".header .bgText").text("CLEANER LOBBY")
        $(".header .text").text("CLEANER LOBBY")
        $("#inviteScreen").fadeOut(250)
        $("#warningScreen").fadeOut(250)
        $("#tutorialScreen").fadeOut(250)
        ...
        $("#counterValue").text(0  + " Cleaned Windows")
        $("#counter").fadeIn(250)
        counterActive = true
        ...
        $("#counterValue").text(event.data.value.toString()  + " Cleaned Windows")
...
function startRandomCleaning() {
    cleaningActive = true
    $(".cleaning").css("cursor", "url('newui/assets/imgs/tool.png') 64 10, auto")
    $(".window").html('<img id="dirt" src="' + "newui/assets/imgs/surface" + (Math.floor(Math.random() * 8) + 1) + ".png" + '" height="100%" width="100%">')
    $('#cleaningProgress').html('0%');
    setTimeout(() => {
        $('#dirt').eraser({
            progressFunction: function(p) {
                $('#cleaningProgress').html(Math.round(p*100)+'%');
            },
            size: 150,
            completeRatio: .95,
            completeFunction: function() {
                $('#cleaningProgress').html('100%');
                setTimeout(() => {
                    $(".cleaning").fadeOut(250)
                    cleaningActive = false
                    $(".cleaning").css("cursor", "auto")
                    $.post(`https://${GetParentResourceName()}/endCleaning`);
                    setTimeout(() => {
                        $(".window").empty()
                    }, 250)
                }, 1000)
...
function closeHUD() {
    ...
    if (cleaningActive) {
        $.post(`https://${GetParentResourceName()}/stopCleaning`);
        $(".cleaning").fadeOut(250)    
    }
