        ...
        if Player ~= nil and Player.Functions ~= nil then
            ...
            for _, item in pairs(Config.RewardItemsToGive) do
                if math.random(100) <= item.chance then
                    if math.floor(itemsToGive * item.amountPerDelivery) >= 1 then
                        Player.Functions.AddItem(item.item_name, math.floor(itemsToGive * item.amountPerDelivery))
                    end
                end
            end
        end
    elseif Config.Framework == "ESX" then
        ...
        if xPlayer ~= nil and xPlayer.addMoney ~= nil then
            ...
            for _, item in pairs(Config.RewardItemsToGive) do
                if math.random(100) <= item.chance then
                    if math.floor(itemsToGive * item.amountPerDelivery) >= 1 then
                        xPlayer.addInventoryItem(item.item_name, math.floor(itemsToGive * item.amountPerDelivery))
                    end
                end
            end
        end
