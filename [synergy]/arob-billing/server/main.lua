...
QBCore.Functions.CreateCallback('arob:billcheck', function(source, cb, args)
    local src = source
    local citizenid = QBCore.Functions.GetPlayer(src).PlayerData.citizenid
    local query = "SELECT * FROM bills WHERE recipient_citizenid = @recipient_citizenid"
    local params = {["@recipient_citizenid"] = citizenid}
    ...
    MySQL.Async.fetchAll(query, params, function(result)
        if result and #result > 0 then
            ...
            for _, data in ipairs(result) do
                ...
                elseif data.status == "Unpaid" then
