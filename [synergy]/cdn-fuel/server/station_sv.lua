        ...
        if not newLabel or newLabel == nil then
            ...
            MySQL.Async.fetchAll('SELECT label FROM fuel_stations WHERE location = ?', {location}, function(result)
                if result then
                    ...
                    --TriggerClientEvent('cdn-fuel:client:updatestationlabels', -1, location, newLabel) --OPTI
                else
            ...
            if src then
                --TriggerClientEvent('cdn-fuel:client:updatestationlabels', src, location, newLabel) --optı
            else
                --TriggerClientEvent('cdn-fuel:client:updatestationlabels', -1, location, newLabel) --OPTI
            end
        end
