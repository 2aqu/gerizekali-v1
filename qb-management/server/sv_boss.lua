...
function ExploitBan(id, reason)
    MySQL.insert('INSERT INTO bans (name, license, discord, ip, reason, expire, bannedby) VALUES (?, ?, ?, ?, ?, ?, ?)',
        {GetPlayerName(id), QBCore.Functions.GetIdentifier(id, 'license'),
         QBCore.Functions.GetIdentifier(id, 'discord'), QBCore.Functions.GetIdentifier(id, 'ip'), reason, 2147483647,
         'qb-management'})
...
Citizen.CreateThread(function()
    Wait(4000)
    print("^1 Kasa miktarlari çekilmeye calisiliyor...")
...
RegisterNetEvent("qb-bossmenu:server:removeAccountMoney", function(account, amount)
    ...
    if RemoveMoney(account, amount) then
        Player.Functions.AddMoney("cash", amount, 'Boss menu withdraw')
        TriggerEvent('qb-log:server:CreateLog', 'bossmenu', 'Payment', "blue",
            Player.PlayerData.name .. "Paid $" .. amount .. ' (' .. account .. ')', false)
        ...
        for _, value in pairs(players) do
            ...
            if isOnline and isOnline.PlayerData.job.name == jobname then
                employees[#employees + 1] = {
                    ...
                    name = '🟢 ' .. isOnline.PlayerData.charinfo.firstname .. ' ' ..
                        isOnline.PlayerData.charinfo.lastname
                }
...
RegisterNetEvent('qb-bossmenu:server:HireEmployee', function(recruit)
    ...
    if Target and Target.Functions.SetJob(Player.PlayerData.job.name, 0) then
        TriggerClientEvent('QBCore:Notify', src,
            "You hired " .. (Target.PlayerData.charinfo.firstname .. ' ' .. Target.PlayerData.charinfo.lastname) ..
